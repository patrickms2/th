...
import { Vips__defectsRepository } from '../repository/Vips__defectsRepository';
import { vips__defectsInputSchema, vips__defectsSchema } from '../entity/Vips__defects';

export const tag = 'Vips__defects';

export default async (app: FastifyInstance) => {
    const schema = TypeBox.Type.Object({
        q: TypeBox.Type.Optional(TypeBox.Type.Partial(vips__defectsSchema, { description: 'Filter query', additionalProperties: false })),
    ...
    app.get<{ Querystring: TypeBox.Static<typeof schema> }>('/vips__defects', {
        // @ts-ignore
        ...
        schema: {
            ...
            tags: [tag],
            summary: 'List vips__defects',
        },
    }, async (req) => {
        // @ts-ignore
        const [items, count] = await getCustomRepository(Vips__defectsRepository).filter(req.query, req.query.page, req.query.limit, req.query.field, req.query.sort.toUpperCase());
        const fileType = req.query.filetype;
        if (fileType && fileType === 'csv') {
            const fields = ['id', 'generated_id','project_version','2=enhancement','defect_name','estimated_hours','actual_hours','description','2=closed','3=in','4=open','5=solved','6=re-open','7=deferred','assign_member','2=medium','3=high','4=very','5=urgent','notes','attachment','attachment_hash',
                'create_user_id','project_id','defect_type','status','assigned_group_id','severity','orders',
    ...
    app.get<{ Querystring: TypeBox.Static<typeof schema> }>('/vips__defects/count', {
        // @ts-ignore
        preHandler: app.auth([app.verifyAuthorized]),
        schema: {
            querystring: schema,
        ...
        // @ts-ignore
        const [items, count] = await getCustomRepository(Vips__defectsRepository).filter(req.query, req.query.page, req.query.limit, req.query.field, req.query.sort.toUpperCase(), true);
        return {
            rows: items,
            count,
            isLastPage: (req.query.page + 1) * req.query.limit >= count,
    ...
    app.get<{
        ...
        schema: {
            ...
            tags: [tag],
            summary: 'Find vips__defects instance to link as relations',
            response: {
        ...
        const repo = getCustomRepository(Vips__defectsRepository);
    ...
    const postPayload = TypeBox.Type.Object({
        data: vips__defectsInputSchema,
    });
    app.post<{
        Body: TypeBox.Static<typeof postPayload>,
        Reply: TypeBox.Static<typeof vips__defectsSchema>
    }>('/vips__defects', {
        // @ts-ignore
        preHandler: app.auth([app.verifyAuthorized]),
        schema: {
            ...
            tags: [tag],
            summary: 'Create new vips__defects',
            body: postPayload,
            response: {
                200: vips__defectsSchema,
                400: TypeBox.Type.Object({
                    statusCode: TypeBox.Type.Number({ default: 400 }),
                    error: TypeBox.Type.Optional(TypeBox.Type.String()),
                    message: TypeBox.Type.String(),
    ...
    app.get<{
        ...
        schema: {
            params: find,
            tags: [tag],
            summary: 'Get specific vips__defects',
            security: [{
                bearerAuth: [],
            }],
            response: {
                ...
                404: TypeBox.Type.Object({
                    ...
                    message: TypeBox.Type.String(),
                }, { description: 'Vips__defects not found' }),
            },
        }
    // @ts-ignore
    }, async (req, reply) => {
        const entity = await getCustomRepository(Vips__defectsRepository).findOne({
            where: {
                id: req.params.id,
            },
        ...
        return entity ? entity : reply.notFound('Vips__defects not found');
    });

    const putSchema = TypeBox.Type.Object({
        id: TypeBox.Type.String({ format: 'uuid' }),
        data: TypeBox.Type.Partial(vips__defectsInputSchema),
    });
    app.put<{
        Params: TypeBox.Static<typeof find>,
        ...
        schema: {
            summary: 'Edit existing vips__defects',
            params: find,
            body: putSchema,
            tags: [tag],
            security: [{
    ...
    app.delete<{
        ...
        preHandler: app.auth([app.verifyAdmin]),
        schema: {
            description: 'Delete vips__defects',
            summary: 'Delete vips__defects',
            params: find,
            tags: [tag],
            security: [{
                bearerAuth: [],
            ...
            response: {
                200: {
                    description: 'vips__defects successfully deleted',
                    type: 'null',
                },
                404: TypeBox.Type.Object({
                    statusCode: TypeBox.Type.Number({ default: 404 }),
        ...
        const entity = await getCustomRepository(Vips__defectsRepository).softDelete(req.params.id);
        if (!entity.affected) {
            reply.notFound('vips__defects not found');
            return;
        }
