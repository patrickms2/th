...
import { Vips__incidentsRepository } from '../repository/Vips__incidentsRepository';
import { vips__incidentsInputSchema, vips__incidentsSchema } from '../entity/Vips__incidents';

export const tag = 'Vips__incidents';

export default async (app: FastifyInstance) => {
    const schema = TypeBox.Type.Object({
        q: TypeBox.Type.Optional(TypeBox.Type.Partial(vips__incidentsSchema, { description: 'Filter query', additionalProperties: false })),
    ...
    app.get<{ Querystring: TypeBox.Static<typeof schema> }>('/vips__incidents', {
        // @ts-ignore
        ...
        schema: {
            ...
            tags: [tag],
            summary: 'List vips__incidents',
        },
    }, async (req) => {
        // @ts-ignore
        const [items, count] = await getCustomRepository(Vips__incidentsRepository).filter(req.query, req.query.page, req.query.limit, req.query.field, req.query.sort.toUpperCase());
        const fileType = req.query.filetype;
        if (fileType && fileType === 'csv') {
            const fields = ['id', 'generated_id','project_version','2=info','incident_name','estimated_hours','actual_hours','description','2=assigned','3=in','4=solved','5=deferred','6=re-open','7=closed','assign_to','2=medium','3=high','4=very','5=urgent','notes',
                'create_user_id','project_id','incident_type','status','assigned_group_id','priority','orders',
    ...
    app.get<{ Querystring: TypeBox.Static<typeof schema> }>('/vips__incidents/count', {
        // @ts-ignore
        preHandler: app.auth([app.verifyAuthorized]),
        schema: {
            querystring: schema,
        ...
        // @ts-ignore
        const [items, count] = await getCustomRepository(Vips__incidentsRepository).filter(req.query, req.query.page, req.query.limit, req.query.field, req.query.sort.toUpperCase(), true);
        return {
            rows: items,
            count,
            isLastPage: (req.query.page + 1) * req.query.limit >= count,
    ...
    app.get<{
        ...
        schema: {
            ...
            tags: [tag],
            summary: 'Find vips__incidents instance to link as relations',
            response: {
        ...
        const repo = getCustomRepository(Vips__incidentsRepository);
    ...
    const postPayload = TypeBox.Type.Object({
        data: vips__incidentsInputSchema,
    });
    app.post<{
        Body: TypeBox.Static<typeof postPayload>,
        Reply: TypeBox.Static<typeof vips__incidentsSchema>
    }>('/vips__incidents', {
        // @ts-ignore
        preHandler: app.auth([app.verifyAuthorized]),
        schema: {
            ...
            tags: [tag],
            summary: 'Create new vips__incidents',
            body: postPayload,
            response: {
                200: vips__incidentsSchema,
                400: TypeBox.Type.Object({
                    statusCode: TypeBox.Type.Number({ default: 400 }),
                    error: TypeBox.Type.Optional(TypeBox.Type.String()),
                    message: TypeBox.Type.String(),
    ...
    app.get<{
        ...
        schema: {
            params: find,
            tags: [tag],
            summary: 'Get specific vips__incidents',
            security: [{
                bearerAuth: [],
            }],
            response: {
                ...
                404: TypeBox.Type.Object({
                    ...
                    message: TypeBox.Type.String(),
                }, { description: 'Vips__incidents not found' }),
            },
        }
    // @ts-ignore
    }, async (req, reply) => {
        const entity = await getCustomRepository(Vips__incidentsRepository).findOne({
            where: {
                id: req.params.id,
            },
        ...
        return entity ? entity : reply.notFound('Vips__incidents not found');
    });

    const putSchema = TypeBox.Type.Object({
        id: TypeBox.Type.String({ format: 'uuid' }),
        data: TypeBox.Type.Partial(vips__incidentsInputSchema),
    });
    app.put<{
        Params: TypeBox.Static<typeof find>,
        ...
        schema: {
            summary: 'Edit existing vips__incidents',
            params: find,
            body: putSchema,
            tags: [tag],
            security: [{
    ...
    app.delete<{
        ...
        preHandler: app.auth([app.verifyAdmin]),
        schema: {
            description: 'Delete vips__incidents',
            summary: 'Delete vips__incidents',
            params: find,
            tags: [tag],
            security: [{
                bearerAuth: [],
            ...
            response: {
                200: {
                    description: 'vips__incidents successfully deleted',
                    type: 'null',
                },
                404: TypeBox.Type.Object({
                    statusCode: TypeBox.Type.Number({ default: 404 }),
        ...
        const entity = await getCustomRepository(Vips__incidentsRepository).softDelete(req.params.id);
        if (!entity.affected) {
            reply.notFound('vips__incidents not found');
            return;
        }
