...
import actions from 'actions/vips__email_template/vips__email_templateListActions';
import actionsVips__email_template from 'actions/vips__email_template/vips__email_templateFormActions';
...
const Vips__email_templateTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'template_name', title: 'template_name' },
    { label: 'template_subject', title: 'template_subject' },
    { label: 'template_body', title: 'template_body' },
    { label: 'type', title: 'type' },
    { label: 'email_group_id', title: 'email_group_id', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.vips__email_template.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__email_template.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__email_template.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__email_template.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__email_template.doUpdate(id, data));
  };
  ...
  const sendVips__email_templateCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__email_template/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__email_templateCSV = async () => {
    const response = await axios({
      url: '/vips__email_template?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__email_templateCSV.csv';
    link.click();
  };
  ...
  const columns = [
    {
      field: 'email_group_id',

      flex: 0.6,
      editable: true,

      headerName: 'email_group_id',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Vips__email_template')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__email_template/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__email_templateCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__email_templateCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__email_template'
                  : 'http://localhost:8080/api-docs/#/Vips__email_template'
              }
            >
              API documentation for vips__email_template
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__email_templateTable;
