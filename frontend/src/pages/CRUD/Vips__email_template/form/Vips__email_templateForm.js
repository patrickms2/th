...
const Vips__email_templateForm = (props) => {
  ...
  const formValidations = () => {
    return FormValidations(vips__email_templateFields, record || {});
  };

  const handleSubmit = (values) => {
    const { id, ...data } = PreparedValues(
      vips__email_templateFields,
      values || {},
    );
    onSubmit(id, data);
  };
  ...
  const title = () => {
    ...
    return isEditing ? 'Edit Vips__email_template' : 'Add Vips__email_template';
  };
          ...
          <form onSubmit={form.handleSubmit}>
            <Grid container spacing={3} direction='column'>
              <Grid item>
                <InputFormItem
                  name={'email_group_id'}
                  schema={vips__email_templateFields}
                />
              </Grid>
