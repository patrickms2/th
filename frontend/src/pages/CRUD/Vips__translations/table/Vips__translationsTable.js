...
import actions from 'actions/vips__translations/vips__translationsListActions';
import actionsVips__translations from 'actions/vips__translations/vips__translationsFormActions';
...
const Vips__translationsTable = () => {
  ...
  const count = useSelector((store) => store.vips__translations.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__translations.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__translations.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__translations.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__translations.doUpdate(id, data));
  };
  ...
  const sendVips__translationsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__translations/bulk-import', data, {
        headers: {
  ...
  const getVips__translationsCSV = async () => {
    const response = await axios({
      url: '/vips__translations?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__translationsCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__translations')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__translations/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__translationsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__translationsCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__translations'
                  : 'http://localhost:8080/api-docs/#/Vips__translations'
              }
            >
              API documentation for vips__translations
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__translationsTable;
