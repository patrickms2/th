...
import actions from 'actions/vips__announcements/vips__announcementsListActions';
import actionsVips__announcements from 'actions/vips__announcements/vips__announcementsFormActions';
...
const Vips__announcementsTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: '1=publish', title: '1=publish' },
    { label: '2=completed', title: '2=completed' },
    { label: '1=all', title: '1=all' },
    { label: 'user_id', title: 'user_id', number: 'true' },
    { label: 'status', title: 'status', number: 'true' },
    { label: 'all_client', title: 'all_client', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.vips__announcements.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__announcements.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__announcements.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__announcements.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__announcements.doUpdate(id, data));
  };
  ...
  const sendVips__announcementsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__announcements/bulk-import', data, {
        headers: {
  ...
  const getVips__announcementsCSV = async () => {
    const response = await axios({
      url: '/vips__announcements?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__announcementsCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__announcements')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__announcements/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__announcementsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__announcementsCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__announcements'
                  : 'http://localhost:8080/api-docs/#/Vips__announcements'
              }
            >
              API documentation for vips__announcements
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__announcementsTable;
