...
import actions from 'actions/vips__payment_gateway_settings/vips__payment_gateway_settingsListActions';
import actionsVips__payment_gateway_settings from 'actions/vips__payment_gateway_settings/vips__payment_gateway_settingsFormActions';
...
const Vips__payment_gateway_settingsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'paypal_checkout_label', title: 'paypal_checkout_label' },
    { label: 'paypal_checkout_client_id', title: 'paypal_checkout_client_id' },
    { label: '0=inactive', title: '0=inactive' },
    { label: 'stripe_label', title: 'stripe_label' },
    { label: 'stripe_api_key', title: 'stripe_api_key' },
    { label: 'stripe_secret_key', title: 'stripe_secret_key' },
    { label: '0=inactive', title: '0=inactive' },
    {
      label: 'paypal_checkout_status',
      title: 'paypal_checkout_status',
      number: 'true',
    },
    { label: 'stripe_status', title: 'stripe_status', number: 'true' },
  ]);
  ...
  const count = useSelector(
    (store) => store.vips__payment_gateway_settings.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.vips__payment_gateway_settings.list.modalOpen,
  );
  const rows = useSelector(
    (store) => store.vips__payment_gateway_settings.list.rows,
  );
  const idToDelete = useSelector(
    (store) => store.vips__payment_gateway_settings.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__payment_gateway_settings.doUpdate(id, data));
  };
  ...
  const sendVips__payment_gateway_settingsCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__payment_gateway_settings/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__payment_gateway_settingsCSV = async () => {
    const response = await axios({
      url: '/vips__payment_gateway_settings?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__payment_gateway_settingsCSV.csv';
    link.click();
  };
  ...
  const columns = [
    {
      field: 'paypal_checkout_label',

      flex: 0.6,
      editable: true,

      headerName: 'paypal_checkout_label',
    },

    {
      field: 'paypal_checkout_client_id',

      flex: 0.6,
      editable: true,

      headerName: 'paypal_checkout_client_id',
    },

    {
      field: 'paypal_checkout_status',

      flex: 0.6,
      editable: true,

      headerName: 'paypal_checkout_status',
    },
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='vips__payment_gateway_settings'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget
        title={humanize('Vips__payment_gateway_settings')}
        disableWidgetMenu
      >
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__payment_gateway_settings/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__payment_gateway_settingsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__payment_gateway_settingsCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__payment_gateway_settings'
                  : 'http://localhost:8080/api-docs/#/Vips__payment_gateway_settings'
              }
            >
              API documentation for vips__payment_gateway_settings
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__payment_gateway_settingsTable;
