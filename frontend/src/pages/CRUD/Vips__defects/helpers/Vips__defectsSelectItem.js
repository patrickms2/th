...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__defects/autocomplete`, { params });
  return response.data;
}

const Vips__defectsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__defects.hasPermissionToCreate,
});

export default connect(select)(Vips__defectsSelectItem);
