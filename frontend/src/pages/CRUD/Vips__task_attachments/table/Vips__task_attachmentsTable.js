...
import * as dataFormat from 'pages/CRUD/Vips__task_attachments/table/Vips__task_attachmentsDataFormatters';

import actions from 'actions/vips__task_attachments/vips__task_attachmentsListActions';
import actionsVips__task_attachments from 'actions/vips__task_attachments/vips__task_attachmentsFormActions';
...
const Vips__task_attachmentsTable = () => {
  ...
  const count = useSelector((store) => store.vips__task_attachments.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__task_attachments.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__task_attachments.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__task_attachments.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__task_attachments.doUpdate(id, data));
  };
  ...
  const sendVips__task_attachmentsCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__task_attachments/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__task_attachmentsCSV = async () => {
    const response = await axios({
      url: '/vips__task_attachments?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__task_attachmentsCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__task_attachments')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__task_attachments/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__task_attachmentsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__task_attachmentsCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__task_attachments'
                  : 'http://localhost:8080/api-docs/#/Vips__task_attachments'
              }
            >
              API documentation for vips__task_attachments
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__task_attachmentsTable;
