...
import * as dataFormat from 'pages/CRUD/Vips__currency/table/Vips__currencyDataFormatters';

import actions from 'actions/vips__currency/vips__currencyListActions';
import actionsVips__currency from 'actions/vips__currency/vips__currencyFormActions';
...
const Vips__currencyTable = () => {
  ...
  const count = useSelector((store) => store.vips__currency.list.count);
  const modalOpen = useSelector((store) => store.vips__currency.list.modalOpen);
  const rows = useSelector((store) => store.vips__currency.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__currency.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__currency.doUpdate(id, data));
  };
  ...
  const sendVips__currencyCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__currency/bulk-import', data, {
        headers: {
  ...
  const getVips__currencyCSV = async () => {
    const response = await axios({
      url: '/vips__currency?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__currencyCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__currency')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__currency/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__currency'
                  : 'http://localhost:8080/api-docs/#/Vips__currency'
              }
            >
              API documentation for vips__currency
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__currencyTable;
