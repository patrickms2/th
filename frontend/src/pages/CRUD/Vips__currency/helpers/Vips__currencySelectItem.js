...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__currency/autocomplete`, { params });
  return response.data;
}

const Vips__currencySelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__currency.hasPermissionToCreate,
});

export default connect(select)(Vips__currencySelectItem);
