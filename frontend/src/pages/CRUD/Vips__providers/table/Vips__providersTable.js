...
import actions from 'actions/vips__providers/vips__providersListActions';
import actionsVips__providers from 'actions/vips__providers/vips__providersFormActions';
...
const Vips__providersTable = () => {
  ...
  const count = useSelector((store) => store.vips__providers.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__providers.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__providers.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__providers.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__providers.doUpdate(id, data));
  };
  ...
  const sendVips__providersCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__providers/bulk-import', data, {
        headers: {
  ...
  const getVips__providersCSV = async () => {
    const response = await axios({
      url: '/vips__providers?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__providersCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__providers')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__providers/new' className={classes.element}>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__providersCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__providers'
                  : 'http://localhost:8080/api-docs/#/Vips__providers'
              }
            >
              API documentation for vips__providers
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__providersTable;
