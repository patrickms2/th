...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__providers/autocomplete`, { params });
  return response.data;
}

const Vips__providersSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__providers.hasPermissionToCreate,
});

export default connect(select)(Vips__providersSelectItem);
