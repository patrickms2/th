...
import * as dataFormat from 'pages/CRUD/Vips__defect_user/table/Vips__defect_userDataFormatters';

import actions from 'actions/vips__defect_user/vips__defect_userListActions';
import actionsVips__defect_user from 'actions/vips__defect_user/vips__defect_userFormActions';
...
const Vips__defect_userTable = () => {
  ...
  const count = useSelector((store) => store.vips__defect_user.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__defect_user.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__defect_user.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__defect_user.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__defect_user.doUpdate(id, data));
  };
  ...
  const sendVips__defect_userCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__defect_user/bulk-import', data, {
        headers: {
  ...
  const getVips__defect_userCSV = async () => {
    const response = await axios({
      url: '/vips__defect_user?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__defect_userCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__defect_user')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__defect_user/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__defect_userCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__defect_userCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__defect_user'
                  : 'http://localhost:8080/api-docs/#/Vips__defect_user'
              }
            >
              API documentation for vips__defect_user
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__defect_userTable;
