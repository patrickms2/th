...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__defect_user/autocomplete`, {
    params,
  });
  return response.data;
}

const Vips__defect_userSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__defect_user.hasPermissionToCreate,
});

export default connect(select)(Vips__defect_userSelectItem);
