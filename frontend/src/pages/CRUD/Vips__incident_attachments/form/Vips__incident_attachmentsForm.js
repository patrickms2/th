...
const Vips__incident_attachmentsForm = (props) => {
  ...
  const formValidations = () => {
    return FormValidations(vips__incident_attachmentsFields, record || {});
  };

  const handleSubmit = (values) => {
    const { id, ...data } = PreparedValues(
      vips__incident_attachmentsFields,
      values || {},
    );
    onSubmit(id, data);
  };
  ...
  const title = () => {
    ...
    return isEditing
      ? 'Edit Vips__incident_attachments'
      : 'Add Vips__incident_attachments';
  };
          ...
          <form onSubmit={form.handleSubmit}>
            <Grid container spacing={3} direction='column'>
              <Grid item>
                <InputFormItem
                  name={'incident_id'}
                  schema={vips__incident_attachmentsFields}
                />
              </Grid>
