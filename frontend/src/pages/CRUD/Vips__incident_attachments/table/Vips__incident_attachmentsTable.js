...
import actions from 'actions/vips__incident_attachments/vips__incident_attachmentsListActions';
import actionsVips__incident_attachments from 'actions/vips__incident_attachments/vips__incident_attachmentsFormActions';
...
const Vips__incident_attachmentsTable = () => {
  ...
  const count = useSelector(
    (store) => store.vips__incident_attachments.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.vips__incident_attachments.list.modalOpen,
  );
  const rows = useSelector(
    (store) => store.vips__incident_attachments.list.rows,
  );
  const idToDelete = useSelector(
    (store) => store.vips__incident_attachments.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__incident_attachments.doUpdate(id, data));
  };
  ...
  const sendVips__incident_attachmentsCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__incident_attachments/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__incident_attachmentsCSV = async () => {
    const response = await axios({
      url: '/vips__incident_attachments?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__incident_attachmentsCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='vips__incident_attachments'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget title={humanize('Vips__incident_attachments')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__incident_attachments/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__incident_attachmentsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__incident_attachmentsCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__incident_attachments'
                  : 'http://localhost:8080/api-docs/#/Vips__incident_attachments'
              }
            >
              API documentation for vips__incident_attachments
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__incident_attachmentsTable;
