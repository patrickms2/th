...
import * as dataFormat from 'pages/CRUD/Vips__user_role_department/table/Vips__user_role_departmentDataFormatters';

import actions from 'actions/vips__user_role_department/vips__user_role_departmentListActions';
import actionsVips__user_role_department from 'actions/vips__user_role_department/vips__user_role_departmentFormActions';
...
const Vips__user_role_departmentTable = () => {
  ...
  const count = useSelector(
    (store) => store.vips__user_role_department.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.vips__user_role_department.list.modalOpen,
  );
  const rows = useSelector(
    (store) => store.vips__user_role_department.list.rows,
  );
  const idToDelete = useSelector(
    (store) => store.vips__user_role_department.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__user_role_department.doUpdate(id, data));
  };
  ...
  const sendVips__user_role_departmentCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__user_role_department/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__user_role_departmentCSV = async () => {
    const response = await axios({
      url: '/vips__user_role_department?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__user_role_departmentCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='vips__user_role_department'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget title={humanize('Vips__user_role_department')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__user_role_department/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__user_role_departmentCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__user_role_departmentCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__user_role_department'
                  : 'http://localhost:8080/api-docs/#/Vips__user_role_department'
              }
            >
              API documentation for vips__user_role_department
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__user_role_departmentTable;
