...
import actions from 'actions/vips__leave_attachments/vips__leave_attachmentsListActions';
import actionsVips__leave_attachments from 'actions/vips__leave_attachments/vips__leave_attachmentsFormActions';
...
const Vips__leave_attachmentsTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'leave_id', title: 'leave_id', number: 'true' },
  ]);
  ...
  const count = useSelector(
    (store) => store.vips__leave_attachments.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.vips__leave_attachments.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__leave_attachments.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__leave_attachments.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__leave_attachments.doUpdate(id, data));
  };
  ...
  const sendVips__leave_attachmentsCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__leave_attachments/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__leave_attachmentsCSV = async () => {
    const response = await axios({
      url: '/vips__leave_attachments?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__leave_attachmentsCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='vips__leave_attachments'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget title={humanize('Vips__leave_attachments')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__leave_attachments/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__leave_attachmentsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__leave_attachmentsCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__leave_attachments'
                  : 'http://localhost:8080/api-docs/#/Vips__leave_attachments'
              }
            >
              API documentation for vips__leave_attachments
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__leave_attachmentsTable;
