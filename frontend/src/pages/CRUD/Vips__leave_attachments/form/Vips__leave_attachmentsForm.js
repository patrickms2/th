...
const Vips__leave_attachmentsForm = (props) => {
  ...
  const formValidations = () => {
    return FormValidations(vips__leave_attachmentsFields, record || {});
  };

  const handleSubmit = (values) => {
    const { id, ...data } = PreparedValues(
      vips__leave_attachmentsFields,
      values || {},
    );
    onSubmit(id, data);
  };
  ...
  const title = () => {
    ...
    return isEditing
      ? 'Edit Vips__leave_attachments'
      : 'Add Vips__leave_attachments';
  };
          ...
          <form onSubmit={form.handleSubmit}>
            <Grid container spacing={3} direction='column'>
              <Grid item>
                <InputFormItem
                  name={'leave_id'}
                  schema={vips__leave_attachmentsFields}
                />
              </Grid>
