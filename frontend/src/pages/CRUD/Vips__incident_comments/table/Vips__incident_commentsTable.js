...
import * as dataFormat from 'pages/CRUD/Vips__incident_comments/table/Vips__incident_commentsDataFormatters';

import actions from 'actions/vips__incident_comments/vips__incident_commentsListActions';
import actionsVips__incident_comments from 'actions/vips__incident_comments/vips__incident_commentsFormActions';
...
const Vips__incident_commentsTable = () => {
  ...
  const count = useSelector(
    (store) => store.vips__incident_comments.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.vips__incident_comments.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__incident_comments.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__incident_comments.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__incident_comments.doUpdate(id, data));
  };
  ...
  const sendVips__incident_commentsCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__incident_comments/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__incident_commentsCSV = async () => {
    const response = await axios({
      url: '/vips__incident_comments?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__incident_commentsCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='vips__incident_comments'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget title={humanize('Vips__incident_comments')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__incident_comments/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__incident_commentsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__incident_commentsCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__incident_comments'
                  : 'http://localhost:8080/api-docs/#/Vips__incident_comments'
              }
            >
              API documentation for vips__incident_comments
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__incident_commentsTable;
