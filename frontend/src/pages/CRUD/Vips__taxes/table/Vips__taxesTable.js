...
import * as dataFormat from 'pages/CRUD/Vips__taxes/table/Vips__taxesDataFormatters';

import actions from 'actions/vips__taxes/vips__taxesListActions';
import actionsVips__taxes from 'actions/vips__taxes/vips__taxesFormActions';
...
const Vips__taxesTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'tax_name', title: 'tax_name' },
    { label: 'tax_rate', title: 'tax_rate' },
  ]);
  ...
  const count = useSelector((store) => store.vips__taxes.list.count);
  const modalOpen = useSelector((store) => store.vips__taxes.list.modalOpen);
  const rows = useSelector((store) => store.vips__taxes.list.rows);
  const idToDelete = useSelector((store) => store.vips__taxes.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsVips__taxes.doUpdate(id, data));
  };
  ...
  const sendVips__taxesCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__taxes/bulk-import', data, {
        headers: {
  ...
  const getVips__taxesCSV = async () => {
    const response = await axios({
      url: '/vips__taxes?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__taxesCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__taxes')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__taxes/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__taxes'
                  : 'http://localhost:8080/api-docs/#/Vips__taxes'
              }
            >
              API documentation for vips__taxes
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__taxesTable;
