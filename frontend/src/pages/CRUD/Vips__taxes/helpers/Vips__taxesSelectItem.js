...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__taxes/autocomplete`, { params });
  return response.data;
}

const Vips__taxesSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__taxes.hasPermissionToCreate,
});

export default connect(select)(Vips__taxesSelectItem);
