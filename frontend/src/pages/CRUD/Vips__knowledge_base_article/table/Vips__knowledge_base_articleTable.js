...
import * as dataFormat from 'pages/CRUD/Vips__knowledge_base_article/table/Vips__knowledge_base_articleDataFormatters';

import actions from 'actions/vips__knowledge_base_article/vips__knowledge_base_articleListActions';
import actionsVips__knowledge_base_article from 'actions/vips__knowledge_base_article/vips__knowledge_base_articleFormActions';
...
const Vips__knowledge_base_articleTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'article_title', title: 'article_title' },
    { label: 'description', title: 'description' },
    { label: 'file_name', title: 'file_name' },
    { label: 'category_id', title: 'category_id', number: 'true' },
  ...
  const count = useSelector(
    (store) => store.vips__knowledge_base_article.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.vips__knowledge_base_article.list.modalOpen,
  );
  const rows = useSelector(
    (store) => store.vips__knowledge_base_article.list.rows,
  );
  const idToDelete = useSelector(
    (store) => store.vips__knowledge_base_article.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__knowledge_base_article.doUpdate(id, data));
  };
  ...
  const sendVips__knowledge_base_articleCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__knowledge_base_article/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__knowledge_base_articleCSV = async () => {
    const response = await axios({
      url: '/vips__knowledge_base_article?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__knowledge_base_articleCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='vips__knowledge_base_article'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget
        title={humanize('Vips__knowledge_base_article')}
        disableWidgetMenu
      >
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__knowledge_base_article/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__knowledge_base_articleCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__knowledge_base_articleCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__knowledge_base_article'
                  : 'http://localhost:8080/api-docs/#/Vips__knowledge_base_article'
              }
            >
              API documentation for vips__knowledge_base_article
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__knowledge_base_articleTable;
