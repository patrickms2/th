...
import actions from 'actions/vips__defects_history/vips__defects_historyListActions';
import actionsVips__defects_history from 'actions/vips__defects_history/vips__defects_historyFormActions';
...
const Vips__defects_historyTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'defect_id', title: 'defect_id', number: 'true' },
    { label: 'created_by_id', title: 'created_by_id', number: 'true' },
    { label: 'commented_by_id', title: 'commented_by_id', number: 'true' },
    { label: 'solved_by_id', title: 'solved_by_id', number: 'true' },
    { label: 'closed_by_id', title: 'closed_by_id', number: 'true' },
    { label: 'updated_by_id', title: 'updated_by_id', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.vips__defects_history.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__defects_history.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__defects_history.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__defects_history.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__defects_history.doUpdate(id, data));
  };
  ...
  const sendVips__defects_historyCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__defects_history/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__defects_historyCSV = async () => {
    const response = await axios({
      url: '/vips__defects_history?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__defects_historyCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      headerName: 'created_by_id',
    },

    {
      field: 'commented_by_id',

      flex: 0.6,
      editable: true,

      headerName: 'commented_by_id',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Vips__defects_history')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__defects_history/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__defects_historyCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__defects_historyCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__defects_history'
                  : 'http://localhost:8080/api-docs/#/Vips__defects_history'
              }
            >
              API documentation for vips__defects_history
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__defects_historyTable;
