...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__items/autocomplete`, { params });
  return response.data;
}

const Vips__itemsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__items.hasPermissionToCreate,
});

export default connect(select)(Vips__itemsSelectItem);
