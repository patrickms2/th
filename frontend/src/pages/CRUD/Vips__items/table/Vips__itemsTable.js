...
import actions from 'actions/vips__items/vips__itemsListActions';
import actionsVips__items from 'actions/vips__items/vips__itemsFormActions';
...
const Vips__itemsTable = () => {
  ...
  const count = useSelector((store) => store.vips__items.list.count);
  const modalOpen = useSelector((store) => store.vips__items.list.modalOpen);
  const rows = useSelector((store) => store.vips__items.list.rows);
  const idToDelete = useSelector((store) => store.vips__items.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsVips__items.doUpdate(id, data));
  };
  ...
  const sendVips__itemsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__items/bulk-import', data, {
        headers: {
  ...
  const getVips__itemsCSV = async () => {
    const response = await axios({
      url: '/vips__items?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__itemsCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__items')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__items/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__items'
                  : 'http://localhost:8080/api-docs/#/Vips__items'
              }
            >
              API documentation for vips__items
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__itemsTable;
