...
import actions from 'actions/vips__item_tax/vips__item_taxListActions';
import actionsVips__item_tax from 'actions/vips__item_tax/vips__item_taxFormActions';
...
const Vips__item_taxTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'item_id', title: 'item_id', number: 'true' },
  ...
  const count = useSelector((store) => store.vips__item_tax.list.count);
  const modalOpen = useSelector((store) => store.vips__item_tax.list.modalOpen);
  const rows = useSelector((store) => store.vips__item_tax.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__item_tax.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__item_tax.doUpdate(id, data));
  };
  ...
  const sendVips__item_taxCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__item_tax/bulk-import', data, {
        headers: {
  ...
  const getVips__item_taxCSV = async () => {
    const response = await axios({
      url: '/vips__item_tax?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__item_taxCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__item_tax')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__item_tax/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__item_tax'
                  : 'http://localhost:8080/api-docs/#/Vips__item_tax'
              }
            >
              API documentation for vips__item_tax
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__item_taxTable;
