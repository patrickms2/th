...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__item_tax/autocomplete`, { params });
  return response.data;
}

const Vips__item_taxSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__item_tax.hasPermissionToCreate,
});

export default connect(select)(Vips__item_taxSelectItem);
