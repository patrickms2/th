...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__projects/autocomplete`, { params });
  return response.data;
}

const Vips__projectsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__projects.hasPermissionToCreate,
});

export default connect(select)(Vips__projectsSelectItem);
