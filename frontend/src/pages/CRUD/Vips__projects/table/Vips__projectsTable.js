...
import actions from 'actions/vips__projects/vips__projectsListActions';
import actionsVips__projects from 'actions/vips__projects/vips__projectsFormActions';
...
const Vips__projectsTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'project_name', title: 'project_name' },
    { label: 'project_version', title: 'project_version' },
    { label: '1=task', title: '1=task' },
    { label: '2=hourly', title: '2=hourly' },
    { label: 'price_rate', title: 'price_rate' },
    { label: 'estimated_hours', title: 'estimated_hours' },
    { label: 'actual_hours', title: 'actual_hours' },
    { label: '2=inprogress', title: '2=inprogress' },
    { label: '3=onhold', title: '3=onhold' },
    { label: '4=cancel', title: '4=cancel' },
    { label: '5=completed', title: '5=completed' },
    { label: 'demo_url', title: 'demo_url' },
    { label: 'description', title: 'description' },
    { label: 'project_logo', title: 'project_logo' },
    { label: 'assign_members', title: 'assign_members' },
    ...
    { label: 'project_hours', title: 'project_hours', number: 'true' },
    { label: 'billing_type', title: 'billing_type', number: 'true' },
    { label: 'status', title: 'status', number: 'true' },
    { label: 'assign_to', title: 'assign_to', number: 'true' },
    { label: 'is_overdue', title: 'is_overdue', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.vips__projects.list.count);
  const modalOpen = useSelector((store) => store.vips__projects.list.modalOpen);
  const rows = useSelector((store) => store.vips__projects.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__projects.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__projects.doUpdate(id, data));
  };
  ...
  const sendVips__projectsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__projects/bulk-import', data, {
        headers: {
  ...
  const getVips__projectsCSV = async () => {
    const response = await axios({
      url: '/vips__projects?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__projectsCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'assign_members',

      flex: 0.6,
      editable: true,

      headerName: 'assign_members',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Vips__projects')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__projects/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__projects'
                  : 'http://localhost:8080/api-docs/#/Vips__projects'
              }
            >
              API documentation for vips__projects
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__projectsTable;
