...
const Vips__slack_integrationForm = (props) => {
  ...
  const formValidations = () => {
    return FormValidations(vips__slack_integrationFields, record || {});
  };

  const handleSubmit = (values) => {
    const { id, ...data } = PreparedValues(
      vips__slack_integrationFields,
      values || {},
    );
    onSubmit(id, data);
  };
  ...
  const title = () => {
    ...
    return isEditing
      ? 'Edit Vips__slack_integration'
      : 'Add Vips__slack_integration';
  };
          ...
          <form onSubmit={form.handleSubmit}>
            <Grid container spacing={3} direction='column'>
              <Grid item>
                <InputFormItem
                  name={'user_id'}
                  schema={vips__slack_integrationFields}
                />
              </Grid>
