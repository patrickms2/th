...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__slack_integration/autocomplete`, {
    params,
  });
  return response.data;
}

const Vips__slack_integrationSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__slack_integration.hasPermissionToCreate,
});

export default connect(select)(Vips__slack_integrationSelectItem);
