...
import actions from 'actions/vips__slack_integration/vips__slack_integrationListActions';
import actionsVips__slack_integration from 'actions/vips__slack_integration/vips__slack_integrationFormActions';
...
const Vips__slack_integrationTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'access_token', title: 'access_token' },
    { label: 'team_id', title: 'team_id' },
    { label: 'channel_id', title: 'channel_id' },
  ...
  const count = useSelector(
    (store) => store.vips__slack_integration.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.vips__slack_integration.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__slack_integration.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__slack_integration.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__slack_integration.doUpdate(id, data));
  };
  ...
  const sendVips__slack_integrationCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__slack_integration/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__slack_integrationCSV = async () => {
    const response = await axios({
      url: '/vips__slack_integration?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__slack_integrationCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='vips__slack_integration'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget title={humanize('Vips__slack_integration')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__slack_integration/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__slack_integrationCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__slack_integrationCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__slack_integration'
                  : 'http://localhost:8080/api-docs/#/Vips__slack_integration'
              }
            >
              API documentation for vips__slack_integration
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__slack_integrationTable;
