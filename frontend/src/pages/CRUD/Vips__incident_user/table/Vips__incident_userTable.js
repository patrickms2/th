...
import * as dataFormat from 'pages/CRUD/Vips__incident_user/table/Vips__incident_userDataFormatters';

import actions from 'actions/vips__incident_user/vips__incident_userListActions';
import actionsVips__incident_user from 'actions/vips__incident_user/vips__incident_userFormActions';
...
const Vips__incident_userTable = () => {
  ...
  const count = useSelector((store) => store.vips__incident_user.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__incident_user.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__incident_user.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__incident_user.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__incident_user.doUpdate(id, data));
  };
  ...
  const sendVips__incident_userCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__incident_user/bulk-import', data, {
        headers: {
  ...
  const getVips__incident_userCSV = async () => {
    const response = await axios({
      url: '/vips__incident_user?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__incident_userCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__incident_user')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__incident_user/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__incident_userCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__incident_userCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__incident_user'
                  : 'http://localhost:8080/api-docs/#/Vips__incident_user'
              }
            >
              API documentation for vips__incident_user
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__incident_userTable;
