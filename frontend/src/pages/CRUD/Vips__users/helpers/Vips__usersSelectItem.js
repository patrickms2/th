...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__users/autocomplete`, { params });
  return response.data;
}

const Vips__usersSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__users.hasPermissionToCreate,
});

export default connect(select)(Vips__usersSelectItem);
