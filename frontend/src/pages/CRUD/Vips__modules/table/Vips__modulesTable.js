...
import actions from 'actions/vips__modules/vips__modulesListActions';
import actionsVips__modules from 'actions/vips__modules/vips__modulesFormActions';
...
const Vips__modulesTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'module_name', title: 'module_name' },
    { label: 'module_slug', title: 'module_slug' },
  ...
  const count = useSelector((store) => store.vips__modules.list.count);
  const modalOpen = useSelector((store) => store.vips__modules.list.modalOpen);
  const rows = useSelector((store) => store.vips__modules.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__modules.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__modules.doUpdate(id, data));
  };
  ...
  const sendVips__modulesCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__modules/bulk-import', data, {
        headers: {
  ...
  const getVips__modulesCSV = async () => {
    const response = await axios({
      url: '/vips__modules?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__modulesCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__modules')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__modules/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__modules'
                  : 'http://localhost:8080/api-docs/#/Vips__modules'
              }
            >
              API documentation for vips__modules
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__modulesTable;
