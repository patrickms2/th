...
import actions from 'actions/vips__department_role_menu/vips__department_role_menuListActions';
import actionsVips__department_role_menu from 'actions/vips__department_role_menu/vips__department_role_menuFormActions';
...
const Vips__department_role_menuTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'department_id', title: 'department_id', number: 'true' },
    { label: 'role_id', title: 'role_id', number: 'true' },
    { label: 'menu_id', title: 'menu_id', number: 'true' },
    { label: 'view', title: 'view', number: 'true' },
    { label: 'created', title: 'created', number: 'true' },
    { label: 'edited', title: 'edited', number: 'true' },
    { label: 'deleted', title: 'deleted', number: 'true' },
  ]);
  ...
  const count = useSelector(
    (store) => store.vips__department_role_menu.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.vips__department_role_menu.list.modalOpen,
  );
  const rows = useSelector(
    (store) => store.vips__department_role_menu.list.rows,
  );
  const idToDelete = useSelector(
    (store) => store.vips__department_role_menu.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__department_role_menu.doUpdate(id, data));
  };
  ...
  const sendVips__department_role_menuCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__department_role_menu/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__department_role_menuCSV = async () => {
    const response = await axios({
      url: '/vips__department_role_menu?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__department_role_menuCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='vips__department_role_menu'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget title={humanize('Vips__department_role_menu')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__department_role_menu/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__department_role_menuCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__department_role_menuCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__department_role_menu'
                  : 'http://localhost:8080/api-docs/#/Vips__department_role_menu'
              }
            >
              API documentation for vips__department_role_menu
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__department_role_menuTable;
