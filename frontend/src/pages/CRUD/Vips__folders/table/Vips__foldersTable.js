...
import * as dataFormat from 'pages/CRUD/Vips__folders/table/Vips__foldersDataFormatters';

import actions from 'actions/vips__folders/vips__foldersListActions';
import actionsVips__folders from 'actions/vips__folders/vips__foldersFormActions';
...
const Vips__foldersTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'folder_name', title: 'folder_name' },
    { label: 'user_id', title: 'user_id', number: 'true' },
    { label: 'parent_folder', title: 'parent_folder', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.vips__folders.list.count);
  const modalOpen = useSelector((store) => store.vips__folders.list.modalOpen);
  const rows = useSelector((store) => store.vips__folders.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__folders.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__folders.doUpdate(id, data));
  };
  ...
  const sendVips__foldersCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__folders/bulk-import', data, {
        headers: {
  ...
  const getVips__foldersCSV = async () => {
    const response = await axios({
      url: '/vips__folders?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__foldersCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__folders')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__folders/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__folders'
                  : 'http://localhost:8080/api-docs/#/Vips__folders'
              }
            >
              API documentation for vips__folders
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__foldersTable;
