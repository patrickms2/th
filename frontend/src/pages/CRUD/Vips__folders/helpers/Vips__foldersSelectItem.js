...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__folders/autocomplete`, { params });
  return response.data;
}

const Vips__foldersSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__folders.hasPermissionToCreate,
});

export default connect(select)(Vips__foldersSelectItem);
