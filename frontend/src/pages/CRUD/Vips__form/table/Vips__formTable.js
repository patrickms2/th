...
import * as dataFormat from 'pages/CRUD/Vips__form/table/Vips__formDataFormatters';

import actions from 'actions/vips__form/vips__formListActions';
import actionsVips__form from 'actions/vips__form/vips__formFormActions';
...
const Vips__formTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'table_name', title: 'table_name' },
  ...
  const count = useSelector((store) => store.vips__form.list.count);
  const modalOpen = useSelector((store) => store.vips__form.list.modalOpen);
  const rows = useSelector((store) => store.vips__form.list.rows);
  const idToDelete = useSelector((store) => store.vips__form.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsVips__form.doUpdate(id, data));
  };
  ...
  const sendVips__formCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__form/bulk-import', data, {
        headers: {
  ...
  const getVips__formCSV = async () => {
    const response = await axios({
      url: '/vips__form?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__formCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__form')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__form/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__form'
                  : 'http://localhost:8080/api-docs/#/Vips__form'
              }
            >
              API documentation for vips__form
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__formTable;
