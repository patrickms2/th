...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__form/autocomplete`, { params });
  return response.data;
}

const Vips__formSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__form.hasPermissionToCreate,
});

export default connect(select)(Vips__formSelectItem);
