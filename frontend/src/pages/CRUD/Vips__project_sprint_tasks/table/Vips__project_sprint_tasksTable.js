...
import actions from 'actions/vips__project_sprint_tasks/vips__project_sprint_tasksListActions';
import actionsVips__project_sprint_tasks from 'actions/vips__project_sprint_tasks/vips__project_sprint_tasksFormActions';
...
const Vips__project_sprint_tasksTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: '2=onhold', title: '2=onhold' },
    { label: '3=closed', title: '3=closed' },
    { label: '4=released|story:1=open', title: '4=released|story:1=open' },
    { label: '2=inprogress', title: '2=inprogress' },
    { label: '3=closed', title: '3=closed' },
    { label: 'type', title: 'type' },
    { label: 'estimated_hours', title: 'estimated_hours' },
    { label: 'project_sprint_id', title: 'project_sprint_id', number: 'true' },
  ...
  const count = useSelector(
    (store) => store.vips__project_sprint_tasks.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.vips__project_sprint_tasks.list.modalOpen,
  );
  const rows = useSelector(
    (store) => store.vips__project_sprint_tasks.list.rows,
  );
  const idToDelete = useSelector(
    (store) => store.vips__project_sprint_tasks.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__project_sprint_tasks.doUpdate(id, data));
  };
  ...
  const sendVips__project_sprint_tasksCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__project_sprint_tasks/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__project_sprint_tasksCSV = async () => {
    const response = await axios({
      url: '/vips__project_sprint_tasks?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__project_sprint_tasksCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: '4=released|story:1=open',

      flex: 0.6,
      editable: true,

      headerName: '4=released|story:1=open',
    },
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='vips__project_sprint_tasks'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget title={humanize('Vips__project_sprint_tasks')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__project_sprint_tasks/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__project_sprint_tasksCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__project_sprint_tasksCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__project_sprint_tasks'
                  : 'http://localhost:8080/api-docs/#/Vips__project_sprint_tasks'
              }
            >
              API documentation for vips__project_sprint_tasks
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__project_sprint_tasksTable;
