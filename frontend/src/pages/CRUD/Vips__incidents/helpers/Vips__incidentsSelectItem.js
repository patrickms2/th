...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__incidents/autocomplete`, { params });
  return response.data;
}

const Vips__incidentsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__incidents.hasPermissionToCreate,
});

export default connect(select)(Vips__incidentsSelectItem);
