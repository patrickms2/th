...
import actions from 'actions/vips__payments/vips__paymentsListActions';
import actionsVips__payments from 'actions/vips__payments/vips__paymentsFormActions';
...
const Vips__paymentsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'receipt_number', title: 'receipt_number' },
    { label: 'transaction_id', title: 'transaction_id' },
    { label: 'payment_method', title: 'payment_method' },
  ...
  const count = useSelector((store) => store.vips__payments.list.count);
  const modalOpen = useSelector((store) => store.vips__payments.list.modalOpen);
  const rows = useSelector((store) => store.vips__payments.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__payments.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__payments.doUpdate(id, data));
  };
  ...
  const sendVips__paymentsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__payments/bulk-import', data, {
        headers: {
  ...
  const getVips__paymentsCSV = async () => {
    const response = await axios({
      url: '/vips__payments?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__paymentsCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      valueGetter: (GridValueGetterParams) =>
        new Date(GridValueGetterParams.row.date),
...

    {
      ...
      editable: true,
      type: 'singleSelect',
      valueOptions: ['in_progress', 'successful', 'failed'],
  ...
  return (
    <div>
      <Widget title={humanize('Vips__payments')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__payments/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__payments'
                  : 'http://localhost:8080/api-docs/#/Vips__payments'
              }
            >
              API documentation for vips__payments
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__paymentsTable;
