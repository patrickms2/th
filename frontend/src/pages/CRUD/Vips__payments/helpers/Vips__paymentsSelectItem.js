...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__payments/autocomplete`, { params });
  return response.data;
}

const Vips__paymentsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__payments.hasPermissionToCreate,
});

export default connect(select)(Vips__paymentsSelectItem);
