...
import actions from 'actions/vips__roles/vips__rolesListActions';
import actionsVips__roles from 'actions/vips__roles/vips__rolesFormActions';
...
const Vips__rolesTable = () => {
  ...
  const count = useSelector((store) => store.vips__roles.list.count);
  const modalOpen = useSelector((store) => store.vips__roles.list.modalOpen);
  const rows = useSelector((store) => store.vips__roles.list.rows);
  const idToDelete = useSelector((store) => store.vips__roles.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsVips__roles.doUpdate(id, data));
  };
  ...
  const sendVips__rolesCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__roles/bulk-import', data, {
        headers: {
  ...
  const getVips__rolesCSV = async () => {
    const response = await axios({
      url: '/vips__roles?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__rolesCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__roles')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__roles/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__roles'
                  : 'http://localhost:8080/api-docs/#/Vips__roles'
              }
            >
              API documentation for vips__roles
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__rolesTable;
