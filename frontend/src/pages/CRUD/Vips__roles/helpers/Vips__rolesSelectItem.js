...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__roles/autocomplete`, { params });
  return response.data;
}

const Vips__rolesSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__roles.hasPermissionToCreate,
});

export default connect(select)(Vips__rolesSelectItem);
