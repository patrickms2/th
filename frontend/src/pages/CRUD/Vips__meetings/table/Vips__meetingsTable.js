...
import actions from 'actions/vips__meetings/vips__meetingsListActions';
import actionsVips__meetings from 'actions/vips__meetings/vips__meetingsFormActions';
...
const Vips__meetingsTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: '3=cancel', title: '3=cancel' },
    { label: '4=completed', title: '4=completed' },
    { label: 'organizer_id', title: 'organizer_id', number: 'true' },
  ...
  const count = useSelector((store) => store.vips__meetings.list.count);
  const modalOpen = useSelector((store) => store.vips__meetings.list.modalOpen);
  const rows = useSelector((store) => store.vips__meetings.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__meetings.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__meetings.doUpdate(id, data));
  };
  ...
  const sendVips__meetingsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__meetings/bulk-import', data, {
        headers: {
  ...
  const getVips__meetingsCSV = async () => {
    const response = await axios({
      url: '/vips__meetings?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__meetingsCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      valueGetter: (GridValueGetterParams) =>
        new Date(GridValueGetterParams.row.start_date),
...

    {
      ...
      type: 'dateTime',
      valueGetter: (GridValueGetterParams) =>
        new Date(GridValueGetterParams.row.end_date),
  ...
  return (
    <div>
      <Widget title={humanize('Vips__meetings')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__meetings/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__meetings'
                  : 'http://localhost:8080/api-docs/#/Vips__meetings'
              }
            >
              API documentation for vips__meetings
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__meetingsTable;
