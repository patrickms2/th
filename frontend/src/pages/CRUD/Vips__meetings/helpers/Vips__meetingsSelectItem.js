...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__meetings/autocomplete`, { params });
  return response.data;
}

const Vips__meetingsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__meetings.hasPermissionToCreate,
});

export default connect(select)(Vips__meetingsSelectItem);
