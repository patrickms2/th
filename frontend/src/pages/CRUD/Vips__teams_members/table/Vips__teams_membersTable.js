...
import * as dataFormat from 'pages/CRUD/Vips__teams_members/table/Vips__teams_membersDataFormatters';

import actions from 'actions/vips__teams_members/vips__teams_membersListActions';
import actionsVips__teams_members from 'actions/vips__teams_members/vips__teams_membersFormActions';
...
const Vips__teams_membersTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'team_id', title: 'team_id', number: 'true' },
  ...
  const count = useSelector((store) => store.vips__teams_members.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__teams_members.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__teams_members.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__teams_members.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__teams_members.doUpdate(id, data));
  };
  ...
  const sendVips__teams_membersCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__teams_members/bulk-import', data, {
        headers: {
  ...
  const getVips__teams_membersCSV = async () => {
    const response = await axios({
      url: '/vips__teams_members?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__teams_membersCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__teams_members')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__teams_members/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__teams_membersCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__teams_membersCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__teams_members'
                  : 'http://localhost:8080/api-docs/#/Vips__teams_members'
              }
            >
              API documentation for vips__teams_members
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__teams_membersTable;
