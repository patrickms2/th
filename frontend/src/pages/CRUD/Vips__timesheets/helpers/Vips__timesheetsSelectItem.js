...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__timesheets/autocomplete`, {
    params,
  });
  return response.data;
}

const Vips__timesheetsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__timesheets.hasPermissionToCreate,
});

export default connect(select)(Vips__timesheetsSelectItem);
