...
import actions from 'actions/vips__timesheets/vips__timesheetsListActions';
import actionsVips__timesheets from 'actions/vips__timesheets/vips__timesheetsFormActions';
...
const Vips__timesheetsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'hour_time', title: 'hour_time' },
    { label: 'note', title: 'note' },
    { label: 'project_id', title: 'project_id', number: 'true' },
    { label: 'module_id', title: 'module_id', number: 'true' },
    { label: 'module_related_id', title: 'module_related_id', number: 'true' },
    { label: 'created_user_id', title: 'created_user_id', number: 'true' },
    { label: 'decimal_time', title: 'decimal_time', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.vips__timesheets.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__timesheets.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__timesheets.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__timesheets.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__timesheets.doUpdate(id, data));
  };
  ...
  const sendVips__timesheetsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__timesheets/bulk-import', data, {
        headers: {
  ...
  const getVips__timesheetsCSV = async () => {
    const response = await axios({
      url: '/vips__timesheets?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__timesheetsCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'module_related_id',

      flex: 0.6,
      editable: true,

      headerName: 'module_related_id',
    },

    {
      ...
      valueGetter: (GridValueGetterParams) =>
        new Date(GridValueGetterParams.row.start_time),
...

    {
      ...
      type: 'dateTime',
      valueGetter: (GridValueGetterParams) =>
        new Date(GridValueGetterParams.row.end_time),
  ...
  return (
    <div>
      <Widget title={humanize('Vips__timesheets')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__timesheets/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__timesheetsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__timesheetsCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__timesheets'
                  : 'http://localhost:8080/api-docs/#/Vips__timesheets'
              }
            >
              API documentation for vips__timesheets
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__timesheetsTable;
