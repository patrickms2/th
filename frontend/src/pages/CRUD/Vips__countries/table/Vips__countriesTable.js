...
import * as dataFormat from 'pages/CRUD/Vips__countries/table/Vips__countriesDataFormatters';

import actions from 'actions/vips__countries/vips__countriesListActions';
import actionsVips__countries from 'actions/vips__countries/vips__countriesFormActions';
...
const Vips__countriesTable = () => {
  ...
  const count = useSelector((store) => store.vips__countries.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__countries.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__countries.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__countries.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__countries.doUpdate(id, data));
  };
  ...
  const sendVips__countriesCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__countries/bulk-import', data, {
        headers: {
  ...
  const getVips__countriesCSV = async () => {
    const response = await axios({
      url: '/vips__countries?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__countriesCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__countries')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__countries/new' className={classes.element}>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__countriesCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__countries'
                  : 'http://localhost:8080/api-docs/#/Vips__countries'
              }
            >
              API documentation for vips__countries
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__countriesTable;
