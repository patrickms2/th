...
import * as dataFormat from 'pages/CRUD/Vips__knowledge_base_category/table/Vips__knowledge_base_categoryDataFormatters';

import actions from 'actions/vips__knowledge_base_category/vips__knowledge_base_categoryListActions';
import actionsVips__knowledge_base_category from 'actions/vips__knowledge_base_category/vips__knowledge_base_categoryFormActions';
...
const Vips__knowledge_base_categoryTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'category_name', title: 'category_name' },
    { label: 'category_logo', title: 'category_logo' },
  ...
  const count = useSelector(
    (store) => store.vips__knowledge_base_category.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.vips__knowledge_base_category.list.modalOpen,
  );
  const rows = useSelector(
    (store) => store.vips__knowledge_base_category.list.rows,
  );
  const idToDelete = useSelector(
    (store) => store.vips__knowledge_base_category.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__knowledge_base_category.doUpdate(id, data));
  };
  ...
  const sendVips__knowledge_base_categoryCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__knowledge_base_category/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__knowledge_base_categoryCSV = async () => {
    const response = await axios({
      url: '/vips__knowledge_base_category?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__knowledge_base_categoryCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='vips__knowledge_base_category'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget
        title={humanize('Vips__knowledge_base_category')}
        disableWidgetMenu
      >
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__knowledge_base_category/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__knowledge_base_categoryCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__knowledge_base_categoryCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__knowledge_base_category'
                  : 'http://localhost:8080/api-docs/#/Vips__knowledge_base_category'
              }
            >
              API documentation for vips__knowledge_base_category
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__knowledge_base_categoryTable;
