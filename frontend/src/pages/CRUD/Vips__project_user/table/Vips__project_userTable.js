...
import actions from 'actions/vips__project_user/vips__project_userListActions';
import actionsVips__project_user from 'actions/vips__project_user/vips__project_userFormActions';
...
const Vips__project_userTable = () => {
  ...
  const count = useSelector((store) => store.vips__project_user.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__project_user.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__project_user.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__project_user.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__project_user.doUpdate(id, data));
  };
  ...
  const sendVips__project_userCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__project_user/bulk-import', data, {
        headers: {
  ...
  const getVips__project_userCSV = async () => {
    const response = await axios({
      url: '/vips__project_user?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__project_userCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__project_user')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__project_user/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__project_userCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__project_userCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__project_user'
                  : 'http://localhost:8080/api-docs/#/Vips__project_user'
              }
            >
              API documentation for vips__project_user
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__project_userTable;
