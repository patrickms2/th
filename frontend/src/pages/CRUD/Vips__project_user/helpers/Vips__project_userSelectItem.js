...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__project_user/autocomplete`, {
    params,
  });
  return response.data;
}

const Vips__project_userSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__project_user.hasPermissionToCreate,
});

export default connect(select)(Vips__project_userSelectItem);
