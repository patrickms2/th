...
import * as dataFormat from 'pages/CRUD/Vips__defect_attachments/table/Vips__defect_attachmentsDataFormatters';

import actions from 'actions/vips__defect_attachments/vips__defect_attachmentsListActions';
import actionsVips__defect_attachments from 'actions/vips__defect_attachments/vips__defect_attachmentsFormActions';
...
const Vips__defect_attachmentsTable = () => {
  ...
  const count = useSelector(
    (store) => store.vips__defect_attachments.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.vips__defect_attachments.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__defect_attachments.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__defect_attachments.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__defect_attachments.doUpdate(id, data));
  };
  ...
  const sendVips__defect_attachmentsCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__defect_attachments/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__defect_attachmentsCSV = async () => {
    const response = await axios({
      url: '/vips__defect_attachments?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__defect_attachmentsCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='vips__defect_attachments'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget title={humanize('Vips__defect_attachments')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__defect_attachments/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__defect_attachmentsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__defect_attachmentsCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__defect_attachments'
                  : 'http://localhost:8080/api-docs/#/Vips__defect_attachments'
              }
            >
              API documentation for vips__defect_attachments
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__defect_attachmentsTable;
