...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__invoices/autocomplete`, { params });
  return response.data;
}

const Vips__invoicesSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__invoices.hasPermissionToCreate,
});

export default connect(select)(Vips__invoicesSelectItem);
