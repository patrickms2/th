...
import actions from 'actions/vips__invoices/vips__invoicesListActions';
import actionsVips__invoices from 'actions/vips__invoices/vips__invoicesFormActions';
...
const Vips__invoicesTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'invoice_number', title: 'invoice_number' },
    { label: 'sub_total', title: 'sub_total' },
    { label: 'total_tax', title: 'total_tax' },
    { label: 'discount', title: 'discount' },
    { label: 'total_discount', title: 'total_discount' },
    { label: 'adjustment', title: 'adjustment' },
    { label: 'total_amount', title: 'total_amount' },
    { label: 'total_due_amount', title: 'total_due_amount' },
    { label: 'reference', title: 'reference' },
    { label: 'note', title: 'note' },
    { label: 'recurrence_pattern', title: 'recurrence_pattern' },
    { label: 'parent_id', title: 'parent_id', number: 'true' },
    { label: 'user_id', title: 'user_id', number: 'true' },
    { label: 'client_id', title: 'client_id', number: 'true' },
    { label: 'project_id', title: 'project_id', number: 'true' },
    { label: 'estimate_id', title: 'estimate_id', number: 'true' },
    { label: 'recurrence', title: 'recurrence', number: 'true' },
    {
      label: 'recurrence_occurrences',
      title: 'recurrence_occurrences',
      number: 'true',
    },
    {
      label: 'is_recurrence_generated',
      title: 'is_recurrence_generated',
      number: 'true',
    },
  ]);
  ...
  const count = useSelector((store) => store.vips__invoices.list.count);
  const modalOpen = useSelector((store) => store.vips__invoices.list.modalOpen);
  const rows = useSelector((store) => store.vips__invoices.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__invoices.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__invoices.doUpdate(id, data));
  };
  ...
  const sendVips__invoicesCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__invoices/bulk-import', data, {
        headers: {
  ...
  const getVips__invoicesCSV = async () => {
    const response = await axios({
      url: '/vips__invoices?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__invoicesCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'recurrence_occurrences',

      flex: 0.6,
      editable: true,

      headerName: 'recurrence_occurrences',
    },

    {
      ...
      headerName: 'recurrence_pattern',
    },

    {
      field: 'is_recurrence_generated',

      flex: 0.6,
      editable: true,

      headerName: 'is_recurrence_generated',
    },

    {
      field: 'last_overdue_reminder_date',

      renderCell: (params) => dataFormat.dateTimeFormatter(params.value),
      editable: true,
      type: 'dateTime',
      valueGetter: (GridValueGetterParams) =>
        new Date(GridValueGetterParams.row.last_overdue_reminder_date),

      headerName: 'last_overdue_reminder_date',
    },

    {
      ...
      editable: true,
      type: 'singleSelect',
      valueOptions: ['paid', 'partially_paid', 'unpaid'],
  ...
  return (
    <div>
      <Widget title={humanize('Vips__invoices')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__invoices/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__invoices'
                  : 'http://localhost:8080/api-docs/#/Vips__invoices'
              }
            >
              API documentation for vips__invoices
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__invoicesTable;
