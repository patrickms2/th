...
import actions from 'actions/vips__project_sprints/vips__project_sprintsListActions';
import actionsVips__project_sprints from 'actions/vips__project_sprints/vips__project_sprintsFormActions';
...
const Vips__project_sprintsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'sprint_name', title: 'sprint_name' },
    { label: 'hours', title: 'hours' },
  ...
  const count = useSelector((store) => store.vips__project_sprints.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__project_sprints.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__project_sprints.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__project_sprints.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__project_sprints.doUpdate(id, data));
  };
  ...
  const sendVips__project_sprintsCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__project_sprints/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__project_sprintsCSV = async () => {
    const response = await axios({
      url: '/vips__project_sprints?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__project_sprintsCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__project_sprints')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__project_sprints/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__project_sprintsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__project_sprintsCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__project_sprints'
                  : 'http://localhost:8080/api-docs/#/Vips__project_sprints'
              }
            >
              API documentation for vips__project_sprints
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__project_sprintsTable;
