...
import actions from 'actions/vips__incident_history/vips__incident_historyListActions';
import actionsVips__incident_history from 'actions/vips__incident_history/vips__incident_historyFormActions';
...
const Vips__incident_historyTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'incident_id', title: 'incident_id', number: 'true' },
  ...
  const count = useSelector((store) => store.vips__incident_history.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__incident_history.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__incident_history.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__incident_history.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__incident_history.doUpdate(id, data));
  };
  ...
  const sendVips__incident_historyCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__incident_history/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__incident_historyCSV = async () => {
    const response = await axios({
      url: '/vips__incident_history?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__incident_historyCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__incident_history')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__incident_history/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__incident_historyCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__incident_historyCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__incident_history'
                  : 'http://localhost:8080/api-docs/#/Vips__incident_history'
              }
            >
              API documentation for vips__incident_history
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__incident_historyTable;
