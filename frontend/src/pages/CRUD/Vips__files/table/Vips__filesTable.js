...
import actions from 'actions/vips__files/vips__filesListActions';
import actionsVips__files from 'actions/vips__files/vips__filesFormActions';
...
const Vips__filesTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'folder_id', title: 'folder_id', number: 'true' },
  ...
  const count = useSelector((store) => store.vips__files.list.count);
  const modalOpen = useSelector((store) => store.vips__files.list.modalOpen);
  const rows = useSelector((store) => store.vips__files.list.rows);
  const idToDelete = useSelector((store) => store.vips__files.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsVips__files.doUpdate(id, data));
  };
  ...
  const sendVips__filesCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__files/bulk-import', data, {
        headers: {
  ...
  const getVips__filesCSV = async () => {
    const response = await axios({
      url: '/vips__files?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__filesCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__files')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__files/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__files'
                  : 'http://localhost:8080/api-docs/#/Vips__files'
              }
            >
              API documentation for vips__files
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__filesTable;
