...
import actions from 'actions/vips__appointment/vips__appointmentListActions';
import actionsVips__appointment from 'actions/vips__appointment/vips__appointmentFormActions';
...
const Vips__appointmentTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: '2=confirmed', title: '2=confirmed' },
    { label: '3=finished', title: '3=finished' },
    { label: '4=canceled', title: '4=canceled' },
    { label: 'user_id', title: 'user_id', number: 'true' },
    { label: 'client_id', title: 'client_id', number: 'true' },
    { label: 'provider_id', title: 'provider_id', number: 'true' },
    ...
    { label: 'send_confirmation', title: 'send_confirmation', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.vips__appointment.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__appointment.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__appointment.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__appointment.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__appointment.doUpdate(id, data));
  };
  ...
  const sendVips__appointmentCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__appointment/bulk-import', data, {
        headers: {
  ...
  const getVips__appointmentCSV = async () => {
    const response = await axios({
      url: '/vips__appointment?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__appointmentCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      valueGetter: (GridValueGetterParams) =>
        new Date(GridValueGetterParams.row.start_date_time),

      headerName: 'start_date_time',
    },

    {
      ...
      type: 'dateTime',
      valueGetter: (GridValueGetterParams) =>
        new Date(GridValueGetterParams.row.end_date_time),

      headerName: 'end_date_time',
    },
...

    {
      ...
      headerName: '4=canceled',
    },

    {
      field: 'send_confirmation',

      flex: 0.6,
      editable: true,

      headerName: 'send_confirmation',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Vips__appointment')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__appointment/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__appointmentCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__appointmentCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__appointment'
                  : 'http://localhost:8080/api-docs/#/Vips__appointment'
              }
            >
              API documentation for vips__appointment
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__appointmentTable;
