...
import * as dataFormat from 'pages/CRUD/Vips__project_sprint_members/table/Vips__project_sprint_membersDataFormatters';

import actions from 'actions/vips__project_sprint_members/vips__project_sprint_membersListActions';
import actionsVips__project_sprint_members from 'actions/vips__project_sprint_members/vips__project_sprint_membersFormActions';
...
const Vips__project_sprint_membersTable = () => {
  ...
  const count = useSelector(
    (store) => store.vips__project_sprint_members.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.vips__project_sprint_members.list.modalOpen,
  );
  const rows = useSelector(
    (store) => store.vips__project_sprint_members.list.rows,
  );
  const idToDelete = useSelector(
    (store) => store.vips__project_sprint_members.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__project_sprint_members.doUpdate(id, data));
  };
  ...
  const sendVips__project_sprint_membersCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__project_sprint_members/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__project_sprint_membersCSV = async () => {
    const response = await axios({
      url: '/vips__project_sprint_members?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__project_sprint_membersCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='vips__project_sprint_members'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget
        title={humanize('Vips__project_sprint_members')}
        disableWidgetMenu
      >
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__project_sprint_members/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__project_sprint_membersCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__project_sprint_membersCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__project_sprint_members'
                  : 'http://localhost:8080/api-docs/#/Vips__project_sprint_members'
              }
            >
              API documentation for vips__project_sprint_members
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__project_sprint_membersTable;
