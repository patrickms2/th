...
import * as dataFormat from 'pages/CRUD/Vips__defect_comments/table/Vips__defect_commentsDataFormatters';

import actions from 'actions/vips__defect_comments/vips__defect_commentsListActions';
import actionsVips__defect_comments from 'actions/vips__defect_comments/vips__defect_commentsFormActions';
...
const Vips__defect_commentsTable = () => {
  ...
  const count = useSelector((store) => store.vips__defect_comments.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__defect_comments.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__defect_comments.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__defect_comments.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__defect_comments.doUpdate(id, data));
  };
  ...
  const sendVips__defect_commentsCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__defect_comments/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__defect_commentsCSV = async () => {
    const response = await axios({
      url: '/vips__defect_comments?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__defect_commentsCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__defect_comments')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__defect_comments/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__defect_commentsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__defect_commentsCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__defect_comments'
                  : 'http://localhost:8080/api-docs/#/Vips__defect_comments'
              }
            >
              API documentation for vips__defect_comments
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__defect_commentsTable;
