...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__defect_comments/autocomplete`, {
    params,
  });
  return response.data;
}

const Vips__defect_commentsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__defect_comments.hasPermissionToCreate,
});

export default connect(select)(Vips__defect_commentsSelectItem);
