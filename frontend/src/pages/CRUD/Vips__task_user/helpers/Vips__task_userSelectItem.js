...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__task_user/autocomplete`, { params });
  return response.data;
}

const Vips__task_userSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__task_user.hasPermissionToCreate,
});

export default connect(select)(Vips__task_userSelectItem);
