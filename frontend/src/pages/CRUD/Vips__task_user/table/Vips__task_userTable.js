...
import * as dataFormat from 'pages/CRUD/Vips__task_user/table/Vips__task_userDataFormatters';

import actions from 'actions/vips__task_user/vips__task_userListActions';
import actionsVips__task_user from 'actions/vips__task_user/vips__task_userFormActions';
...
const Vips__task_userTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'edit', title: 'edit', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.vips__task_user.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__task_user.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__task_user.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__task_user.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__task_user.doUpdate(id, data));
  };
  ...
  const sendVips__task_userCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__task_user/bulk-import', data, {
        headers: {
  ...
  const getVips__task_userCSV = async () => {
    const response = await axios({
      url: '/vips__task_user?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__task_userCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__task_user')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__task_user/new' className={classes.element}>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__task_userCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__task_user'
                  : 'http://localhost:8080/api-docs/#/Vips__task_user'
              }
            >
              API documentation for vips__task_user
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__task_userTable;
