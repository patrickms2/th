...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__database_backups/autocomplete`, {
    params,
  });
  return response.data;
}

const Vips__database_backupsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__database_backups.hasPermissionToCreate,
});

export default connect(select)(Vips__database_backupsSelectItem);
