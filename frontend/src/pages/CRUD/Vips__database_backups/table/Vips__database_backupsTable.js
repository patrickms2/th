...
import * as dataFormat from 'pages/CRUD/Vips__database_backups/table/Vips__database_backupsDataFormatters';

import actions from 'actions/vips__database_backups/vips__database_backupsListActions';
import actionsVips__database_backups from 'actions/vips__database_backups/vips__database_backupsFormActions';
...
const Vips__database_backupsTable = () => {
  ...
  const count = useSelector((store) => store.vips__database_backups.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__database_backups.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__database_backups.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__database_backups.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__database_backups.doUpdate(id, data));
  };
  ...
  const sendVips__database_backupsCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__database_backups/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__database_backupsCSV = async () => {
    const response = await axios({
      url: '/vips__database_backups?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__database_backupsCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__database_backups')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__database_backups/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__database_backupsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__database_backupsCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__database_backups'
                  : 'http://localhost:8080/api-docs/#/Vips__database_backups'
              }
            >
              API documentation for vips__database_backups
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__database_backupsTable;
