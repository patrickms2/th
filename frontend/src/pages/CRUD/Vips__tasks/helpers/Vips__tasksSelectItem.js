...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__tasks/autocomplete`, { params });
  return response.data;
}

const Vips__tasksSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__tasks.hasPermissionToCreate,
});

export default connect(select)(Vips__tasksSelectItem);
