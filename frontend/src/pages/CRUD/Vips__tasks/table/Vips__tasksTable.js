...
import actions from 'actions/vips__tasks/vips__tasksListActions';
import actionsVips__tasks from 'actions/vips__tasks/vips__tasksFormActions';
...
const Vips__tasksTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: '2=in', title: '2=in' },
    { label: '3=on', title: '3=on' },
    { label: '4=waiting', title: '4=waiting' },
    { label: '5=cancel', title: '5=cancel' },
    { label: '6=completed', title: '6=completed' },
    { label: '2=very', title: '2=very' },
    { label: '3=high', title: '3=high' },
    { label: '4=medium', title: '4=medium' },
    { label: '5=low', title: '5=low' },
    { label: 'description', title: 'description' },
    { label: 'notes', title: 'notes' },
    { label: 'parent_task_id', title: 'parent_task_id', number: 'true' },
    ...
    { label: 'order', title: 'order', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.vips__tasks.list.count);
  const modalOpen = useSelector((store) => store.vips__tasks.list.modalOpen);
  const rows = useSelector((store) => store.vips__tasks.list.rows);
  const idToDelete = useSelector((store) => store.vips__tasks.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsVips__tasks.doUpdate(id, data));
  };
  ...
  const sendVips__tasksCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__tasks/bulk-import', data, {
        headers: {
  ...
  const getVips__tasksCSV = async () => {
    const response = await axios({
      url: '/vips__tasks?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__tasksCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      headerName: 'planned_start_date',
    },

    {
      field: 'planned_end_date',

      headerName: 'planned_end_date',
    },

    {
      ...
      valueGetter: (GridValueGetterParams) =>
        new Date(GridValueGetterParams.row.task_start_date),

      headerName: 'task_start_date',
    },

    {
      ...
      type: 'dateTime',
      valueGetter: (GridValueGetterParams) =>
        new Date(GridValueGetterParams.row.task_end_date),

      headerName: 'task_end_date',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Vips__tasks')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__tasks/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__tasks'
                  : 'http://localhost:8080/api-docs/#/Vips__tasks'
              }
            >
              API documentation for vips__tasks
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__tasksTable;
