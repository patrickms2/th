...
import actions from 'actions/vips__project_comments/vips__project_commentsListActions';
import actionsVips__project_comments from 'actions/vips__project_comments/vips__project_commentsFormActions';
...
const Vips__project_commentsTable = () => {
  ...
  const count = useSelector((store) => store.vips__project_comments.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__project_comments.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__project_comments.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__project_comments.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__project_comments.doUpdate(id, data));
  };
  ...
  const sendVips__project_commentsCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__project_comments/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__project_commentsCSV = async () => {
    const response = await axios({
      url: '/vips__project_comments?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__project_commentsCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__project_comments')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__project_comments/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__project_commentsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__project_commentsCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__project_comments'
                  : 'http://localhost:8080/api-docs/#/Vips__project_comments'
              }
            >
              API documentation for vips__project_comments
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__project_commentsTable;
