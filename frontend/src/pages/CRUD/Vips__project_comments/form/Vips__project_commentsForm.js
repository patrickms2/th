...
const Vips__project_commentsForm = (props) => {
  ...
  const formValidations = () => {
    return FormValidations(vips__project_commentsFields, record || {});
  };

  const handleSubmit = (values) => {
    const { id, ...data } = PreparedValues(
      vips__project_commentsFields,
      values || {},
    );
    onSubmit(id, data);
  };
  ...
  const title = () => {
    ...
    return isEditing
      ? 'Edit Vips__project_comments'
      : 'Add Vips__project_comments';
  };
          ...
          <form onSubmit={form.handleSubmit}>
            <Grid container spacing={3} direction='column'>
              <Grid item>
                <InputFormItem
                  name={'user_id'}
                  schema={vips__project_commentsFields}
                />
              </Grid>
