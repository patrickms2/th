...
import * as dataFormat from 'pages/CRUD/Vips__teams/table/Vips__teamsDataFormatters';

import actions from 'actions/vips__teams/vips__teamsListActions';
import actionsVips__teams from 'actions/vips__teams/vips__teamsFormActions';
...
const Vips__teamsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'team_name', title: 'team_name' },
    { label: 'description', title: 'description' },
    { label: 'team_leader', title: 'team_leader', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.vips__teams.list.count);
  const modalOpen = useSelector((store) => store.vips__teams.list.modalOpen);
  const rows = useSelector((store) => store.vips__teams.list.rows);
  const idToDelete = useSelector((store) => store.vips__teams.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsVips__teams.doUpdate(id, data));
  };
  ...
  const sendVips__teamsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__teams/bulk-import', data, {
        headers: {
  ...
  const getVips__teamsCSV = async () => {
    const response = await axios({
      url: '/vips__teams?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__teamsCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__teams')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__teams/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__teams'
                  : 'http://localhost:8080/api-docs/#/Vips__teams'
              }
            >
              API documentation for vips__teams
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__teamsTable;
