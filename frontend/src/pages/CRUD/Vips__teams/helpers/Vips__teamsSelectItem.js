...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__teams/autocomplete`, { params });
  return response.data;
}

const Vips__teamsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__teams.hasPermissionToCreate,
});

export default connect(select)(Vips__teamsSelectItem);
