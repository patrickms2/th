...
import * as dataFormat from 'pages/CRUD/Vips__email_template_groups/table/Vips__email_template_groupsDataFormatters';

import actions from 'actions/vips__email_template_groups/vips__email_template_groupsListActions';
import actionsVips__email_template_groups from 'actions/vips__email_template_groups/vips__email_template_groupsFormActions';
...
const Vips__email_template_groupsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'email_group_name', title: 'email_group_name' },
  ]);
  ...
  const count = useSelector(
    (store) => store.vips__email_template_groups.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.vips__email_template_groups.list.modalOpen,
  );
  const rows = useSelector(
    (store) => store.vips__email_template_groups.list.rows,
  );
  const idToDelete = useSelector(
    (store) => store.vips__email_template_groups.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__email_template_groups.doUpdate(id, data));
  };
  ...
  const sendVips__email_template_groupsCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__email_template_groups/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__email_template_groupsCSV = async () => {
    const response = await axios({
      url: '/vips__email_template_groups?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__email_template_groupsCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='vips__email_template_groups'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget title={humanize('Vips__email_template_groups')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__email_template_groups/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__email_template_groupsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__email_template_groupsCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__email_template_groups'
                  : 'http://localhost:8080/api-docs/#/Vips__email_template_groups'
              }
            >
              API documentation for vips__email_template_groups
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__email_template_groupsTable;
