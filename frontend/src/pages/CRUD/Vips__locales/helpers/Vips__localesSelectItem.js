...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__locales/autocomplete`, { params });
  return response.data;
}

const Vips__localesSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__locales.hasPermissionToCreate,
});

export default connect(select)(Vips__localesSelectItem);
