...
import * as dataFormat from 'pages/CRUD/Vips__locales/table/Vips__localesDataFormatters';

import actions from 'actions/vips__locales/vips__localesListActions';
import actionsVips__locales from 'actions/vips__locales/vips__localesFormActions';
...
const Vips__localesTable = () => {
  ...
  const count = useSelector((store) => store.vips__locales.list.count);
  const modalOpen = useSelector((store) => store.vips__locales.list.modalOpen);
  const rows = useSelector((store) => store.vips__locales.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__locales.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__locales.doUpdate(id, data));
  };
  ...
  const sendVips__localesCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__locales/bulk-import', data, {
        headers: {
  ...
  const getVips__localesCSV = async () => {
    const response = await axios({
      url: '/vips__locales?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__localesCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__locales')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__locales/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__locales'
                  : 'http://localhost:8080/api-docs/#/Vips__locales'
              }
            >
              API documentation for vips__locales
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__localesTable;
