...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__timer_logs/autocomplete`, {
    params,
  });
  return response.data;
}

const Vips__timer_logsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__timer_logs.hasPermissionToCreate,
});

export default connect(select)(Vips__timer_logsSelectItem);
