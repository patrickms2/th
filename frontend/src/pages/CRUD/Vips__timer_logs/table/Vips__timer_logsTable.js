...
import actions from 'actions/vips__timer_logs/vips__timer_logsListActions';
import actionsVips__timer_logs from 'actions/vips__timer_logs/vips__timer_logsFormActions';
...
const Vips__timer_logsTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'reference_id', title: 'reference_id', number: 'true' },
    { label: 'is_task_timer', title: 'is_task_timer', number: 'true' },
    { label: 'is_defect_timer', title: 'is_defect_timer', number: 'true' },
    { label: 'is_incident_timer', title: 'is_incident_timer', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.vips__timer_logs.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__timer_logs.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__timer_logs.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__timer_logs.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__timer_logs.doUpdate(id, data));
  };
  ...
  const sendVips__timer_logsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__timer_logs/bulk-import', data, {
        headers: {
  ...
  const getVips__timer_logsCSV = async () => {
    const response = await axios({
      url: '/vips__timer_logs?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__timer_logsCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      headerName: 'is_task_timer',
    },

    {
      field: 'is_defect_timer',

      flex: 0.6,
      editable: true,

      headerName: 'is_defect_timer',
    },

    {
      field: 'is_incident_timer',

      flex: 0.6,
      editable: true,

      headerName: 'is_incident_timer',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Vips__timer_logs')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__timer_logs/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__timer_logsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__timer_logsCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__timer_logs'
                  : 'http://localhost:8080/api-docs/#/Vips__timer_logs'
              }
            >
              API documentation for vips__timer_logs
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__timer_logsTable;
