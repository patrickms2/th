...
import * as dataFormat from 'pages/CRUD/Vips__task_comments/table/Vips__task_commentsDataFormatters';

import actions from 'actions/vips__task_comments/vips__task_commentsListActions';
import actionsVips__task_comments from 'actions/vips__task_comments/vips__task_commentsFormActions';
...
const Vips__task_commentsTable = () => {
  ...
  const count = useSelector((store) => store.vips__task_comments.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__task_comments.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__task_comments.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__task_comments.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__task_comments.doUpdate(id, data));
  };
  ...
  const sendVips__task_commentsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__task_comments/bulk-import', data, {
        headers: {
  ...
  const getVips__task_commentsCSV = async () => {
    const response = await axios({
      url: '/vips__task_comments?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__task_commentsCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__task_comments')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__task_comments/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__task_commentsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__task_commentsCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__task_comments'
                  : 'http://localhost:8080/api-docs/#/Vips__task_comments'
              }
            >
              API documentation for vips__task_comments
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__task_commentsTable;
