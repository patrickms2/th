...
import actions from 'actions/vips__estimate_settings/vips__estimate_settingsListActions';
import actionsVips__estimate_settings from 'actions/vips__estimate_settings/vips__estimate_settingsFormActions';
...
const Vips__estimate_settingsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'estimate_prefix', title: 'estimate_prefix' },
    { label: 'estimate_logo', title: 'estimate_logo' },
    ...
    {
      label: 'auto_remind_before',
      title: 'auto_remind_before',
      number: 'true',
    },
    {
      label: 'auto_convert_to_invoice_on_client_accept',
      title: 'auto_convert_to_invoice_on_client_accept',
      number: 'true',
    },
  ]);
  ...
  const count = useSelector(
    (store) => store.vips__estimate_settings.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.vips__estimate_settings.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__estimate_settings.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__estimate_settings.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__estimate_settings.doUpdate(id, data));
  };
  ...
  const sendVips__estimate_settingsCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__estimate_settings/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__estimate_settingsCSV = async () => {
    const response = await axios({
      url: '/vips__estimate_settings?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__estimate_settingsCSV.csv';
    link.click();
  };
  ...
  const columns = [
    {
      field: 'estimate_prefix',

      flex: 0.6,
      editable: true,

      headerName: 'estimate_prefix',
    },
    ...
    {
      field: 'auto_remind_before',

      flex: 0.6,
      editable: true,

      headerName: 'auto_remind_before',
    },

    {
      field: 'auto_convert_to_invoice_on_client_accept',

      flex: 0.6,
      editable: true,

      headerName: 'auto_convert_to_invoice_on_client_accept',
    },

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='vips__estimate_settings'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget title={humanize('Vips__estimate_settings')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__estimate_settings/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__estimate_settingsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__estimate_settingsCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__estimate_settings'
                  : 'http://localhost:8080/api-docs/#/Vips__estimate_settings'
              }
            >
              API documentation for vips__estimate_settings
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__estimate_settingsTable;
