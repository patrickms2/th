...
import * as dataFormat from 'pages/CRUD/Vips__departments_roles/table/Vips__departments_rolesDataFormatters';

import actions from 'actions/vips__departments_roles/vips__departments_rolesListActions';
import actionsVips__departments_roles from 'actions/vips__departments_roles/vips__departments_rolesFormActions';
...
const Vips__departments_rolesTable = () => {
  ...
  const count = useSelector(
    (store) => store.vips__departments_roles.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.vips__departments_roles.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__departments_roles.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__departments_roles.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__departments_roles.doUpdate(id, data));
  };
  ...
  const sendVips__departments_rolesCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__departments_roles/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__departments_rolesCSV = async () => {
    const response = await axios({
      url: '/vips__departments_roles?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__departments_rolesCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='vips__departments_roles'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget title={humanize('Vips__departments_roles')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__departments_roles/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__departments_rolesCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__departments_rolesCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__departments_roles'
                  : 'http://localhost:8080/api-docs/#/Vips__departments_roles'
              }
            >
              API documentation for vips__departments_roles
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__departments_rolesTable;
