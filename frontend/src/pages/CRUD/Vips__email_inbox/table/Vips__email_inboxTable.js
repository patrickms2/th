...
import actions from 'actions/vips__email_inbox/vips__email_inboxListActions';
import actionsVips__email_inbox from 'actions/vips__email_inbox/vips__email_inboxFormActions';
...
const Vips__email_inboxTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'message_body', title: 'message_body' },
    { label: 'type', title: 'type' },
    { label: 'sent', title: 'sent' },
    { label: 'draft', title: 'draft' },
    { label: 'trash', title: 'trash' },
    { label: 'view_status', title: 'view_status' },
    { label: 'favourites', title: 'favourites' },
    { label: '1=yes', title: '1=yes' },
    { label: 'notify_me', title: 'notify_me' },
    { label: '0=off', title: '0=off' },
  ...
  const count = useSelector((store) => store.vips__email_inbox.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__email_inbox.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__email_inbox.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__email_inbox.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__email_inbox.doUpdate(id, data));
  };
  ...
  const sendVips__email_inboxCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__email_inbox/bulk-import', data, {
        headers: {
  ...
  const getVips__email_inboxCSV = async () => {
    const response = await axios({
      url: '/vips__email_inbox?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__email_inboxCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__email_inbox')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__email_inbox/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__email_inboxCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__email_inboxCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__email_inbox'
                  : 'http://localhost:8080/api-docs/#/Vips__email_inbox'
              }
            >
              API documentation for vips__email_inbox
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__email_inboxTable;
