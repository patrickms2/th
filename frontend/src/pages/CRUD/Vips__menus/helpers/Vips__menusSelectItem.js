...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__menus/autocomplete`, { params });
  return response.data;
}

const Vips__menusSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__menus.hasPermissionToCreate,
});

export default connect(select)(Vips__menusSelectItem);
