...
import actions from 'actions/vips__menus/vips__menusListActions';
import actionsVips__menus from 'actions/vips__menus/vips__menusFormActions';
...
const Vips__menusTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: '0=unsubscription', title: '0=unsubscription' },
    { label: '0=inactive', title: '0=inactive' },
    { label: 'parent_menu_id', title: 'parent_menu_id', number: 'true' },
    { label: 'order', title: 'order', number: 'true' },
    { label: 'subscription', title: 'subscription', number: 'true' },
  ...
  const count = useSelector((store) => store.vips__menus.list.count);
  const modalOpen = useSelector((store) => store.vips__menus.list.modalOpen);
  const rows = useSelector((store) => store.vips__menus.list.rows);
  const idToDelete = useSelector((store) => store.vips__menus.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsVips__menus.doUpdate(id, data));
  };
  ...
  const sendVips__menusCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__menus/bulk-import', data, {
        headers: {
  ...
  const getVips__menusCSV = async () => {
    const response = await axios({
      url: '/vips__menus?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__menusCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: '0=unsubscription',

      flex: 0.6,
      editable: true,

      headerName: '0=unsubscription',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Vips__menus')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__menus/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__menus'
                  : 'http://localhost:8080/api-docs/#/Vips__menus'
              }
            >
              API documentation for vips__menus
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__menusTable;
