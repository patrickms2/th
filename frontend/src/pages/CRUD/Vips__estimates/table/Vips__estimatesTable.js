...
import actions from 'actions/vips__estimates/vips__estimatesListActions';
import actionsVips__estimates from 'actions/vips__estimates/vips__estimatesFormActions';
...
const Vips__estimatesTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'estimate_number', title: 'estimate_number' },
  ...
  const count = useSelector((store) => store.vips__estimates.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__estimates.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__estimates.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__estimates.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__estimates.doUpdate(id, data));
  };
  ...
  const sendVips__estimatesCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__estimates/bulk-import', data, {
        headers: {
  ...
  const getVips__estimatesCSV = async () => {
    const response = await axios({
      url: '/vips__estimates?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__estimatesCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      headerName: 'estimate_number',
    },

    {
      field: 'estimate_date',

      headerName: 'estimate_date',
    },

    {
      field: 'valid_till',

      headerName: 'valid_till',
    },
...

    {
      ...
      editable: true,
      type: 'singleSelect',
      valueOptions: ['draft', 'sent', 'expired', 'declined', 'accepted'],
  ...
  return (
    <div>
      <Widget title={humanize('Vips__estimates')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__estimates/new' className={classes.element}>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__estimatesCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__estimates'
                  : 'http://localhost:8080/api-docs/#/Vips__estimates'
              }
            >
              API documentation for vips__estimates
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__estimatesTable;
