...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__estimates/autocomplete`, { params });
  return response.data;
}

const Vips__estimatesSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__estimates.hasPermissionToCreate,
});

export default connect(select)(Vips__estimatesSelectItem);
