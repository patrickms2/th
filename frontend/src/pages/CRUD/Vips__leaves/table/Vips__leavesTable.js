...
import actions from 'actions/vips__leaves/vips__leavesListActions';
import actionsVips__leaves from 'actions/vips__leaves/vips__leavesFormActions';
...
const Vips__leavesTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'duration', title: 'duration' },
    { label: 'half', title: 'half' },
    { label: 'duration_type', title: 'duration_type' },
    { label: 'second_half', title: 'second_half' },
    { label: 'leave_day', title: 'leave_day' },
    { label: 'reason', title: 'reason' },
    { label: 'reject_reason', title: 'reject_reason' },
    { label: '2=approved', title: '2=approved' },
    { label: '3=rejected', title: '3=rejected' },
    { label: '4=cancel', title: '4=cancel' },
    { label: '5=taken', title: '5=taken' },
    { label: 'user_id', title: 'user_id', number: 'true' },
    { label: 'leave_type_id', title: 'leave_type_id', number: 'true' },
  ...
  const count = useSelector((store) => store.vips__leaves.list.count);
  const modalOpen = useSelector((store) => store.vips__leaves.list.modalOpen);
  const rows = useSelector((store) => store.vips__leaves.list.rows);
  const idToDelete = useSelector((store) => store.vips__leaves.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsVips__leaves.doUpdate(id, data));
  };
  ...
  const sendVips__leavesCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__leaves/bulk-import', data, {
        headers: {
  ...
  const getVips__leavesCSV = async () => {
    const response = await axios({
      url: '/vips__leaves?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__leavesCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      headerName: 'leave_day',
    },

    {
      field: 'leave_date',

      headerName: 'leave_date',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Vips__leaves')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__leaves/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__leaves'
                  : 'http://localhost:8080/api-docs/#/Vips__leaves'
              }
            >
              API documentation for vips__leaves
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__leavesTable;
