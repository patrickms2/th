...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__leaves/autocomplete`, { params });
  return response.data;
}

const Vips__leavesSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__leaves.hasPermissionToCreate,
});

export default connect(select)(Vips__leavesSelectItem);
