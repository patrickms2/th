...
import * as dataFormat from 'pages/CRUD/Vips__email_inbox_attachment/table/Vips__email_inbox_attachmentDataFormatters';

import actions from 'actions/vips__email_inbox_attachment/vips__email_inbox_attachmentListActions';
import actionsVips__email_inbox_attachment from 'actions/vips__email_inbox_attachment/vips__email_inbox_attachmentFormActions';
...
const Vips__email_inbox_attachmentTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'file_extension', title: 'file_extension' },
    { label: 'file_hash', title: 'file_hash' },
    { label: 'mime_type', title: 'mime_type' },
    { label: 'mailbox_id', title: 'mailbox_id', number: 'true' },
    { label: 'file_size', title: 'file_size', number: 'true' },
  ]);
  ...
  const count = useSelector(
    (store) => store.vips__email_inbox_attachment.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.vips__email_inbox_attachment.list.modalOpen,
  );
  const rows = useSelector(
    (store) => store.vips__email_inbox_attachment.list.rows,
  );
  const idToDelete = useSelector(
    (store) => store.vips__email_inbox_attachment.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__email_inbox_attachment.doUpdate(id, data));
  };
  ...
  const sendVips__email_inbox_attachmentCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__email_inbox_attachment/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__email_inbox_attachmentCSV = async () => {
    const response = await axios({
      url: '/vips__email_inbox_attachment?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__email_inbox_attachmentCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='vips__email_inbox_attachment'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget
        title={humanize('Vips__email_inbox_attachment')}
        disableWidgetMenu
      >
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__email_inbox_attachment/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__email_inbox_attachmentCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__email_inbox_attachmentCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__email_inbox_attachment'
                  : 'http://localhost:8080/api-docs/#/Vips__email_inbox_attachment'
              }
            >
              API documentation for vips__email_inbox_attachment
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__email_inbox_attachmentTable;
