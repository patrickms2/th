...
import * as dataFormat from 'pages/CRUD/Vips__project_attachments/table/Vips__project_attachmentsDataFormatters';

import actions from 'actions/vips__project_attachments/vips__project_attachmentsListActions';
import actionsVips__project_attachments from 'actions/vips__project_attachments/vips__project_attachmentsFormActions';
...
const Vips__project_attachmentsTable = () => {
  ...
  const count = useSelector(
    (store) => store.vips__project_attachments.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.vips__project_attachments.list.modalOpen,
  );
  const rows = useSelector(
    (store) => store.vips__project_attachments.list.rows,
  );
  const idToDelete = useSelector(
    (store) => store.vips__project_attachments.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__project_attachments.doUpdate(id, data));
  };
  ...
  const sendVips__project_attachmentsCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__project_attachments/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__project_attachmentsCSV = async () => {
    const response = await axios({
      url: '/vips__project_attachments?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__project_attachmentsCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='vips__project_attachments'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget title={humanize('Vips__project_attachments')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__project_attachments/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__project_attachmentsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__project_attachmentsCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__project_attachments'
                  : 'http://localhost:8080/api-docs/#/Vips__project_attachments'
              }
            >
              API documentation for vips__project_attachments
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__project_attachmentsTable;
