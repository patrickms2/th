...
import actions from 'actions/vips__leave_types/vips__leave_typesListActions';
import actionsVips__leave_types from 'actions/vips__leave_types/vips__leave_typesFormActions';
...
const Vips__leave_typesTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'leave_type', title: 'leave_type' },
    { label: 'color', title: 'color' },
    { label: 'no_of_leaves', title: 'no_of_leaves', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.vips__leave_types.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__leave_types.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__leave_types.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__leave_types.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__leave_types.doUpdate(id, data));
  };
  ...
  const sendVips__leave_typesCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__leave_types/bulk-import', data, {
        headers: {
  ...
  const getVips__leave_typesCSV = async () => {
    const response = await axios({
      url: '/vips__leave_types?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__leave_typesCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__leave_types')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__leave_types/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__leave_typesCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__leave_typesCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__leave_types'
                  : 'http://localhost:8080/api-docs/#/Vips__leave_types'
              }
            >
              API documentation for vips__leave_types
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__leave_typesTable;
