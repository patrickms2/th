...
const Vips__leave_typesForm = (props) => {
  ...
  const formValidations = () => {
    return FormValidations(vips__leave_typesFields, record || {});
  };

  const handleSubmit = (values) => {
    const { id, ...data } = PreparedValues(
      vips__leave_typesFields,
      values || {},
    );
    onSubmit(id, data);
  };
  ...
  const title = () => {
    ...
    return isEditing ? 'Edit Vips__leave_types' : 'Add Vips__leave_types';
  };
          ...
          <form onSubmit={form.handleSubmit}>
            <Grid container spacing={3} direction='column'>
              <Grid item>
                <InputFormItem
                  name={'leave_type'}
                  schema={vips__leave_typesFields}
                  autoFocus
                />
              </Grid>
