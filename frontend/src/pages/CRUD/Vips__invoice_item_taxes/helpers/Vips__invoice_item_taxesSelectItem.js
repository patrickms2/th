...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__invoice_item_taxes/autocomplete`, {
    params,
  });
  return response.data;
}

const Vips__invoice_item_taxesSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__invoice_item_taxes.hasPermissionToCreate,
});

export default connect(select)(Vips__invoice_item_taxesSelectItem);
