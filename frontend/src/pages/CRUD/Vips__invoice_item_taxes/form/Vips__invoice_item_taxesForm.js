...
const Vips__invoice_item_taxesForm = (props) => {
  ...
  const formValidations = () => {
    return FormValidations(vips__invoice_item_taxesFields, record || {});
  };

  const handleSubmit = (values) => {
    const { id, ...data } = PreparedValues(
      vips__invoice_item_taxesFields,
      values || {},
    );
    onSubmit(id, data);
  };
  ...
  const title = () => {
    ...
    return isEditing
      ? 'Edit Vips__invoice_item_taxes'
      : 'Add Vips__invoice_item_taxes';
  };
            ...
            <Grid container spacing={3} direction='column'>
              <Grid item>
                <InputFormItem
                  ...
                  schema={vips__invoice_item_taxesFields}
                />
              </Grid>
