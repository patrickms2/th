...
import actions from 'actions/vips__invoice_item_taxes/vips__invoice_item_taxesListActions';
import actionsVips__invoice_item_taxes from 'actions/vips__invoice_item_taxes/vips__invoice_item_taxesFormActions';
...
const Vips__invoice_item_taxesTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'invoice_item_id', title: 'invoice_item_id', number: 'true' },
  ...
  const count = useSelector(
    (store) => store.vips__invoice_item_taxes.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.vips__invoice_item_taxes.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__invoice_item_taxes.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__invoice_item_taxes.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__invoice_item_taxes.doUpdate(id, data));
  };
  ...
  const sendVips__invoice_item_taxesCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__invoice_item_taxes/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__invoice_item_taxesCSV = async () => {
    const response = await axios({
      url: '/vips__invoice_item_taxes?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__invoice_item_taxesCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='vips__invoice_item_taxes'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget title={humanize('Vips__invoice_item_taxes')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__invoice_item_taxes/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__invoice_item_taxesCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__invoice_item_taxesCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__invoice_item_taxes'
                  : 'http://localhost:8080/api-docs/#/Vips__invoice_item_taxes'
              }
            >
              API documentation for vips__invoice_item_taxes
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__invoice_item_taxesTable;
