...
import actions from 'actions/vips__notifications/vips__notificationsListActions';
import actionsVips__notifications from 'actions/vips__notifications/vips__notificationsFormActions';
...
const Vips__notificationsTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'route_related_id', title: 'route_related_id', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.vips__notifications.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__notifications.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__notifications.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__notifications.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__notifications.doUpdate(id, data));
  };
  ...
  const sendVips__notificationsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__notifications/bulk-import', data, {
        headers: {
  ...
  const getVips__notificationsCSV = async () => {
    const response = await axios({
      url: '/vips__notifications?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__notificationsCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'route_related_id',

      flex: 0.6,
      editable: true,

      headerName: 'route_related_id',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Vips__notifications')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__notifications/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__notificationsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__notificationsCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__notifications'
                  : 'http://localhost:8080/api-docs/#/Vips__notifications'
              }
            >
              API documentation for vips__notifications
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__notificationsTable;
