...
const Vips__notificationsForm = (props) => {
  ...
  const formValidations = () => {
    return FormValidations(vips__notificationsFields, record || {});
  };

  const handleSubmit = (values) => {
    const { id, ...data } = PreparedValues(
      vips__notificationsFields,
      values || {},
    );
    onSubmit(id, data);
  };
  ...
  const title = () => {
    ...
    return isEditing ? 'Edit Vips__notifications' : 'Add Vips__notifications';
  };
          ...
          <form onSubmit={form.handleSubmit}>
            <Grid container spacing={3} direction='column'>
              <Grid item>
                <InputFormItem
                  name={'user_id'}
                  schema={vips__notificationsFields}
                />
              </Grid>
