...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__holidays/autocomplete`, { params });
  return response.data;
}

const Vips__holidaysSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__holidays.hasPermissionToCreate,
});

export default connect(select)(Vips__holidaysSelectItem);
