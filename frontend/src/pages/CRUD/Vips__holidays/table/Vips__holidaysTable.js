...
import * as dataFormat from 'pages/CRUD/Vips__holidays/table/Vips__holidaysDataFormatters';

import actions from 'actions/vips__holidays/vips__holidaysListActions';
import actionsVips__holidays from 'actions/vips__holidays/vips__holidaysFormActions';
...
const Vips__holidaysTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'event_name', title: 'event_name' },
  ...
  const count = useSelector((store) => store.vips__holidays.list.count);
  const modalOpen = useSelector((store) => store.vips__holidays.list.modalOpen);
  const rows = useSelector((store) => store.vips__holidays.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__holidays.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__holidays.doUpdate(id, data));
  };
  ...
  const sendVips__holidaysCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__holidays/bulk-import', data, {
        headers: {
  ...
  const getVips__holidaysCSV = async () => {
    const response = await axios({
      url: '/vips__holidays?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__holidaysCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__holidays')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__holidays/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__holidays'
                  : 'http://localhost:8080/api-docs/#/Vips__holidays'
              }
            >
              API documentation for vips__holidays
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__holidaysTable;
