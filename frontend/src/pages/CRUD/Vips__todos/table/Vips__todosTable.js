...
import actions from 'actions/vips__todos/vips__todosListActions';
import actionsVips__todos from 'actions/vips__todos/vips__todosFormActions';
...
const Vips__todosTable = () => {
  ...
  const count = useSelector((store) => store.vips__todos.list.count);
  const modalOpen = useSelector((store) => store.vips__todos.list.modalOpen);
  const rows = useSelector((store) => store.vips__todos.list.rows);
  const idToDelete = useSelector((store) => store.vips__todos.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsVips__todos.doUpdate(id, data));
  };
  ...
  const sendVips__todosCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__todos/bulk-import', data, {
        headers: {
  ...
  const getVips__todosCSV = async () => {
    const response = await axios({
      url: '/vips__todos?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__todosCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__todos')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__todos/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__todos'
                  : 'http://localhost:8080/api-docs/#/Vips__todos'
              }
            >
              API documentation for vips__todos
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__todosTable;
