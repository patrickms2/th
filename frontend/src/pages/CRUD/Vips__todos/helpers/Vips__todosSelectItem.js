...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__todos/autocomplete`, { params });
  return response.data;
}

const Vips__todosSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__todos.hasPermissionToCreate,
});

export default connect(select)(Vips__todosSelectItem);
