...
import actions from 'actions/vips__custom_fields/vips__custom_fieldsListActions';
import actionsVips__custom_fields from 'actions/vips__custom_fields/vips__custom_fieldsFormActions';
...
const Vips__custom_fieldsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'field_label', title: 'field_label' },
    { label: 'field_column', title: 'field_column' },
    { label: 'default_value', title: 'default_value' },
    { label: 'help_text', title: 'help_text' },
    { label: 'true=active', title: 'true=active' },
    { label: 'form_id', title: 'form_id', number: 'true' },
    { label: 'is_required', title: 'is_required', number: 'true' },
    { label: 'status', title: 'status', number: 'true' },
    { label: 'show_on_details', title: 'show_on_details', number: 'true' },
    { label: 'field_type', title: 'field_type', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.vips__custom_fields.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__custom_fields.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__custom_fields.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__custom_fields.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__custom_fields.doUpdate(id, data));
  };
  ...
  const sendVips__custom_fieldsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__custom_fields/bulk-import', data, {
        headers: {
  ...
  const getVips__custom_fieldsCSV = async () => {
    const response = await axios({
      url: '/vips__custom_fields?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__custom_fieldsCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      headerName: 'true=active',
    },

    {
      field: 'show_on_details',

      flex: 0.6,
      editable: true,

      headerName: 'show_on_details',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Vips__custom_fields')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__custom_fields/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__custom_fieldsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__custom_fieldsCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__custom_fields'
                  : 'http://localhost:8080/api-docs/#/Vips__custom_fields'
              }
            >
              API documentation for vips__custom_fields
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__custom_fieldsTable;
