...
import * as dataFormat from 'pages/CRUD/Vips__appointment_recurrence/table/Vips__appointment_recurrenceDataFormatters';

import actions from 'actions/vips__appointment_recurrence/vips__appointment_recurrenceListActions';
import actionsVips__appointment_recurrence from 'actions/vips__appointment_recurrence/vips__appointment_recurrenceFormActions';
...
const Vips__appointment_recurrenceTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'appointment_id', title: 'appointment_id', number: 'true' },
  ...
  const count = useSelector(
    (store) => store.vips__appointment_recurrence.list.count,
  );
  const modalOpen = useSelector(
    (store) => store.vips__appointment_recurrence.list.modalOpen,
  );
  const rows = useSelector(
    (store) => store.vips__appointment_recurrence.list.rows,
  );
  const idToDelete = useSelector(
    (store) => store.vips__appointment_recurrence.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__appointment_recurrence.doUpdate(id, data));
  };
  ...
  const sendVips__appointment_recurrenceCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__appointment_recurrence/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__appointment_recurrenceCSV = async () => {
    const response = await axios({
      url: '/vips__appointment_recurrence?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__appointment_recurrenceCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      renderCell: (params) => (
        <Actions
          classes={classes}
          entity='vips__appointment_recurrence'
          openModal={openModal}
          {...params}
        />
      ),
  ...
  return (
    <div>
      <Widget
        title={humanize('Vips__appointment_recurrence')}
        disableWidgetMenu
      >
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__appointment_recurrence/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__appointment_recurrenceCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__appointment_recurrenceCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__appointment_recurrence'
                  : 'http://localhost:8080/api-docs/#/Vips__appointment_recurrence'
              }
            >
              API documentation for vips__appointment_recurrence
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__appointment_recurrenceTable;
