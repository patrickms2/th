...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__password_resets/autocomplete`, {
    params,
  });
  return response.data;
}

const Vips__password_resetsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__password_resets.hasPermissionToCreate,
});

export default connect(select)(Vips__password_resetsSelectItem);
