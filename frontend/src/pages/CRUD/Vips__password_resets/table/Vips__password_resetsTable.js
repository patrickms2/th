...
import actions from 'actions/vips__password_resets/vips__password_resetsListActions';
import actionsVips__password_resets from 'actions/vips__password_resets/vips__password_resetsFormActions';
...
const Vips__password_resetsTable = () => {
  ...
  const count = useSelector((store) => store.vips__password_resets.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__password_resets.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__password_resets.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__password_resets.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__password_resets.doUpdate(id, data));
  };
  ...
  const sendVips__password_resetsCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__password_resets/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__password_resetsCSV = async () => {
    const response = await axios({
      url: '/vips__password_resets?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__password_resetsCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__password_resets')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__password_resets/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__password_resetsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__password_resetsCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__password_resets'
                  : 'http://localhost:8080/api-docs/#/Vips__password_resets'
              }
            >
              API documentation for vips__password_resets
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__password_resetsTable;
