...
import actions from 'actions/vips__user_activities/vips__user_activitiesListActions';
import actionsVips__user_activities from 'actions/vips__user_activities/vips__user_activitiesFormActions';
...
const Vips__user_activitiesTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'ip_address', title: 'ip_address' },
    { label: 'diff_for_humans', title: 'diff_for_humans' },
    { label: 'user_id', title: 'user_id', number: 'true' },
    { label: 'self_parent_id', title: 'self_parent_id', number: 'true' },
    { label: 'module_field_id', title: 'module_field_id', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.vips__user_activities.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__user_activities.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__user_activities.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__user_activities.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__user_activities.doUpdate(id, data));
  };
  ...
  const sendVips__user_activitiesCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__user_activities/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__user_activitiesCSV = async () => {
    const response = await axios({
      url: '/vips__user_activities?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__user_activitiesCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'self_parent_id',

      flex: 0.6,
      editable: true,

      headerName: 'self_parent_id',
    },
    ...
    {
      field: 'diff_for_humans',

      flex: 0.6,
      editable: true,

      headerName: 'diff_for_humans',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Vips__user_activities')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__user_activities/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__user_activitiesCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__user_activitiesCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__user_activities'
                  : 'http://localhost:8080/api-docs/#/Vips__user_activities'
              }
            >
              API documentation for vips__user_activities
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__user_activitiesTable;
