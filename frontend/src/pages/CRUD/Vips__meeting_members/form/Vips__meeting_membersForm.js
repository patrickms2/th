...
const Vips__meeting_membersForm = (props) => {
  ...
  const formValidations = () => {
    return FormValidations(vips__meeting_membersFields, record || {});
  };

  const handleSubmit = (values) => {
    const { id, ...data } = PreparedValues(
      vips__meeting_membersFields,
      values || {},
    );
    onSubmit(id, data);
  };
  ...
  const title = () => {
    ...
    return isEditing
      ? 'Edit Vips__meeting_members'
      : 'Add Vips__meeting_members';
  };
          ...
          <form onSubmit={form.handleSubmit}>
            <Grid container spacing={3} direction='column'>
              <Grid item>
                <InputFormItem
                  name={'user_id'}
                  schema={vips__meeting_membersFields}
                />
              </Grid>
