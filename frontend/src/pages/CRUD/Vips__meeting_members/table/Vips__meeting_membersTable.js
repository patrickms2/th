...
import actions from 'actions/vips__meeting_members/vips__meeting_membersListActions';
import actionsVips__meeting_members from 'actions/vips__meeting_members/vips__meeting_membersFormActions';
...
const Vips__meeting_membersTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'meeting_id', title: 'meeting_id', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.vips__meeting_members.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__meeting_members.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__meeting_members.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__meeting_members.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__meeting_members.doUpdate(id, data));
  };
  ...
  const sendVips__meeting_membersCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__meeting_members/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__meeting_membersCSV = async () => {
    const response = await axios({
      url: '/vips__meeting_members?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__meeting_membersCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__meeting_members')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__meeting_members/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__meeting_membersCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__meeting_membersCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__meeting_members'
                  : 'http://localhost:8080/api-docs/#/Vips__meeting_members'
              }
            >
              API documentation for vips__meeting_members
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__meeting_membersTable;
