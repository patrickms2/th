...
import * as dataFormat from 'pages/CRUD/Vips__payment_methods/table/Vips__payment_methodsDataFormatters';

import actions from 'actions/vips__payment_methods/vips__payment_methodsListActions';
import actionsVips__payment_methods from 'actions/vips__payment_methods/vips__payment_methodsFormActions';
...
const Vips__payment_methodsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'method_name', title: 'method_name' },
    { label: 'description', title: 'description' },
    { label: '0=inactive', title: '0=inactive' },
    { label: 'show_on_pdf', title: 'show_on_pdf', number: 'true' },
  ...
  const count = useSelector((store) => store.vips__payment_methods.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__payment_methods.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__payment_methods.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__payment_methods.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__payment_methods.doUpdate(id, data));
  };
  ...
  const sendVips__payment_methodsCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__payment_methods/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__payment_methodsCSV = async () => {
    const response = await axios({
      url: '/vips__payment_methods?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__payment_methodsCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__payment_methods')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__payment_methods/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__payment_methodsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__payment_methodsCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__payment_methods'
                  : 'http://localhost:8080/api-docs/#/Vips__payment_methods'
              }
            >
              API documentation for vips__payment_methods
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__payment_methodsTable;
