...
import actions from 'actions/vips__clients/vips__clientsListActions';
import actionsVips__clients from 'actions/vips__clients/vips__clientsFormActions';
...
const Vips__clientsTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'company_mobile', title: 'company_mobile' },
    { label: 'company_zipcode', title: 'company_zipcode' },
    { label: 'company_city', title: 'company_city' },
    { label: 'company_country', title: 'company_country' },
    { label: 'company_fax', title: 'company_fax' },
    { label: 'company_address', title: 'company_address' },
    { label: 'website', title: 'website' },
    { label: 'skype_id', title: 'skype_id' },
    { label: 'facebook', title: 'facebook' },
    { label: 'twitter', title: 'twitter' },
    { label: 'linkedin', title: 'linkedin' },
    { label: 'hosting_company', title: 'hosting_company' },
    { label: 'host_name', title: 'host_name' },
    { label: 'host_username', title: 'host_username' },
    { label: 'host_password', title: 'host_password' },
    { label: 'host_port', title: 'host_port' },
  ...
  const count = useSelector((store) => store.vips__clients.list.count);
  const modalOpen = useSelector((store) => store.vips__clients.list.modalOpen);
  const rows = useSelector((store) => store.vips__clients.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__clients.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__clients.doUpdate(id, data));
  };
  ...
  const sendVips__clientsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__clients/bulk-import', data, {
        headers: {
  ...
  const getVips__clientsCSV = async () => {
    const response = await axios({
      url: '/vips__clients?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__clientsCSV.csv';
    link.click();
  };
  ...
  const columns = [
    ...
    {
      field: 'hosting_company',

      flex: 0.6,
      editable: true,

      headerName: 'hosting_company',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Vips__clients')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__clients/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__clients'
                  : 'http://localhost:8080/api-docs/#/Vips__clients'
              }
            >
              API documentation for vips__clients
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__clientsTable;
