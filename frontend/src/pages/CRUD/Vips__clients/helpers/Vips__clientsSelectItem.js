...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/vips__clients/autocomplete`, { params });
  return response.data;
}

const Vips__clientsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.vips__clients.hasPermissionToCreate,
});

export default connect(select)(Vips__clientsSelectItem);
