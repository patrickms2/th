...
import actions from 'actions/vips__slack_settings/vips__slack_settingsListActions';
import actionsVips__slack_settings from 'actions/vips__slack_settings/vips__slack_settingsFormActions';
...
const Vips__slack_settingsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'slack_client_id', title: 'slack_client_id' },
    { label: 'slack_client_secret', title: 'slack_client_secret' },
    { label: 'slack_redirect_url', title: 'slack_redirect_url' },
    { label: '0=inactive', title: '0=inactive' },
    { label: 'is_authenticate', title: 'is_authenticate', number: 'true' },
    { label: 'slack_status', title: 'slack_status', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.vips__slack_settings.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__slack_settings.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__slack_settings.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__slack_settings.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__slack_settings.doUpdate(id, data));
  };
  ...
  const sendVips__slack_settingsCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__slack_settings/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__slack_settingsCSV = async () => {
    const response = await axios({
      url: '/vips__slack_settings?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__slack_settingsCSV.csv';
    link.click();
  };
  ...
  const columns = [
    {
      field: 'slack_client_id',

      flex: 0.6,
      editable: true,

      headerName: 'slack_client_id',
    },

    {
      field: 'slack_client_secret',

      flex: 0.6,
      editable: true,

      headerName: 'slack_client_secret',
    },

    {
      field: 'slack_redirect_url',

      flex: 0.6,
      editable: true,

      headerName: 'slack_redirect_url',
    },

    {
      field: 'is_authenticate',

      flex: 0.6,
      editable: true,

      headerName: 'is_authenticate',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Vips__slack_settings')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__slack_settings/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__slack_settingsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__slack_settingsCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__slack_settings'
                  : 'http://localhost:8080/api-docs/#/Vips__slack_settings'
              }
            >
              API documentation for vips__slack_settings
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__slack_settingsTable;
