...
import * as dataFormat from 'pages/CRUD/Vips__languages/table/Vips__languagesDataFormatters';

import actions from 'actions/vips__languages/vips__languagesListActions';
import actionsVips__languages from 'actions/vips__languages/vips__languagesFormActions';
...
const Vips__languagesTable = () => {
  ...
  const count = useSelector((store) => store.vips__languages.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__languages.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__languages.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__languages.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__languages.doUpdate(id, data));
  };
  ...
  const sendVips__languagesCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__languages/bulk-import', data, {
        headers: {
  ...
  const getVips__languagesCSV = async () => {
    const response = await axios({
      url: '/vips__languages?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__languagesCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__languages')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__languages/new' className={classes.element}>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__languagesCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__languages'
                  : 'http://localhost:8080/api-docs/#/Vips__languages'
              }
            >
              API documentation for vips__languages
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__languagesTable;
