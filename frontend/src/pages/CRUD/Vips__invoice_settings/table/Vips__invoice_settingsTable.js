...
import actions from 'actions/vips__invoice_settings/vips__invoice_settingsListActions';
import actionsVips__invoice_settings from 'actions/vips__invoice_settings/vips__invoice_settingsFormActions';
...
const Vips__invoice_settingsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'invoice_prefix', title: 'invoice_prefix' },
    { label: 'invoice_logo', title: 'invoice_logo' },
    { label: 'template', title: 'template' },
    { label: 'gst_number', title: 'gst_number' },
    { label: 'client_note', title: 'client_note' },
    { label: 'terms_conditions', title: 'terms_conditions' },
    { label: 'auto_remind_after', title: 'auto_remind_after' },
    { label: '3', title: '3' },
    { label: 'payment_prefix', title: 'payment_prefix' },
    { label: 'due_after', title: 'due_after', number: 'true' },
    { label: 'show_gst_number', title: 'show_gst_number', number: 'true' },
    { label: 'auto_remind', title: 'auto_remind', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.vips__invoice_settings.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__invoice_settings.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__invoice_settings.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__invoice_settings.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__invoice_settings.doUpdate(id, data));
  };
  ...
  const sendVips__invoice_settingsCSV = async () => {
    ...
    try {
      const result = await axios.post(
        'vips__invoice_settings/bulk-import',
        data,
        {
          headers: {
  ...
  const getVips__invoice_settingsCSV = async () => {
    const response = await axios({
      url: '/vips__invoice_settings?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__invoice_settingsCSV.csv';
    link.click();
  };
  ...
  const columns = [
...

    {
      ...
      headerName: 'gst_number',
    },

    {
      field: 'show_gst_number',

      flex: 0.6,
      editable: true,

      headerName: 'show_gst_number',
    },
...

    {
      ...
      headerName: 'auto_remind',
    },

    {
      field: 'auto_remind_after',

      flex: 0.6,
      editable: true,

      headerName: 'auto_remind_after',
    },
  ...
  return (
    <div>
      <Widget title={humanize('Vips__invoice_settings')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link
            to='/admin/vips__invoice_settings/new'
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__invoice_settingsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__invoice_settingsCSV()}
            >
              Confirm
            </Button>
            <Button
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin +
                    '/api-docs/#/Vips__invoice_settings'
                  : 'http://localhost:8080/api-docs/#/Vips__invoice_settings'
              }
            >
              API documentation for vips__invoice_settings
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__invoice_settingsTable;
