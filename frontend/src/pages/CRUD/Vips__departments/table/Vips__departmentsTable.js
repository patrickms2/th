...
import * as dataFormat from 'pages/CRUD/Vips__departments/table/Vips__departmentsDataFormatters';

import actions from 'actions/vips__departments/vips__departmentsListActions';
import actionsVips__departments from 'actions/vips__departments/vips__departmentsFormActions';
...
const Vips__departmentsTable = () => {
  ...
  const count = useSelector((store) => store.vips__departments.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__departments.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__departments.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__departments.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__departments.doUpdate(id, data));
  };
  ...
  const sendVips__departmentsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__departments/bulk-import', data, {
        headers: {
  ...
  const getVips__departmentsCSV = async () => {
    const response = await axios({
      url: '/vips__departments?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__departmentsCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__departments')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__departments/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__departmentsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__departmentsCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__departments'
                  : 'http://localhost:8080/api-docs/#/Vips__departments'
              }
            >
              API documentation for vips__departments
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__departmentsTable;
