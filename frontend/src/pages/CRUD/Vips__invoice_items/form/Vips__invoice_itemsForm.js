...
const Vips__invoice_itemsForm = (props) => {
  ...
  const formValidations = () => {
    return FormValidations(vips__invoice_itemsFields, record || {});
  };

  const handleSubmit = (values) => {
    const { id, ...data } = PreparedValues(
      vips__invoice_itemsFields,
      values || {},
    );
    onSubmit(id, data);
  };
  ...
  const title = () => {
    ...
    return isEditing ? 'Edit Vips__invoice_items' : 'Add Vips__invoice_items';
  };
          ...
          <form onSubmit={form.handleSubmit}>
            <Grid container spacing={3} direction='column'>
              <Grid item>
                <InputFormItem
                  name={'invoice_id'}
                  schema={vips__invoice_itemsFields}
                />
              </Grid>
