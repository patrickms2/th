...
import actions from 'actions/vips__invoice_items/vips__invoice_itemsListActions';
import actionsVips__invoice_items from 'actions/vips__invoice_items/vips__invoice_itemsFormActions';
...
const Vips__invoice_itemsTable = () => {
  ...
  const count = useSelector((store) => store.vips__invoice_items.list.count);
  const modalOpen = useSelector(
    (store) => store.vips__invoice_items.list.modalOpen,
  );
  const rows = useSelector((store) => store.vips__invoice_items.list.rows);
  const idToDelete = useSelector(
    (store) => store.vips__invoice_items.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsVips__invoice_items.doUpdate(id, data));
  };
  ...
  const sendVips__invoice_itemsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('vips__invoice_items/bulk-import', data, {
        headers: {
  ...
  const getVips__invoice_itemsCSV = async () => {
    const response = await axios({
      url: '/vips__invoice_items?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'vips__invoice_itemsCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Vips__invoice_items')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/vips__invoice_items/new' className={classes.element}>
          ...
          <Button
            type='button'
            variant='contained'
            onClick={getVips__invoice_itemsCSV}
            className={classes.element}
          >
            Export CSV
          </Button>
        ...
        >
          <Box sx={styleForModal}>
            ...
            <Button
              ...
              variant='contained'
              className={classes.element}
              onClick={() => sendVips__invoice_itemsCSV()}
            >
              Confirm
            </Button>
            <Button
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Vips__invoice_items'
                  : 'http://localhost:8080/api-docs/#/Vips__invoice_items'
              }
            >
              API documentation for vips__invoice_items
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Vips__invoice_itemsTable;
